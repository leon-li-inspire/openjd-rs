...
#[test] fn default_is_10_million() { assert_eq!(DEFAULT_OPERATION_LIMIT, 10_000_000); }
...
#[test] fn range_iterations_count() {
    let e = eval_op("range(100)", 50).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  range(100)\n", "  ^~~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn list_comprehension_iterations_count() {
    let e = eval_op("[x for x in range(1000)]", 50).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  [x for x in range(1000)]\n", "              ^~~~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn sum_iterations_count() {
    let e = eval_op("sum(range(1000))", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  sum(range(1000))\n", "      ^~~~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn min_iterations_count() {
    let e = eval_op("min(range(1000))", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  min(range(1000))\n", "      ^~~~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn max_iterations_count() {
    let e = eval_op("max(range(1000))", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  max(range(1000))\n", "      ^~~~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn sorted_iterations_count() {
    let e = eval_op("sorted(range(1000))", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  sorted(range(1000))\n", "         ^~~~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn reversed_iterations_count() {
    let e = eval_op("reversed(range(1000))", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  reversed(range(1000))\n", "           ^~~~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn join_iterations_count() {
    let e = eval_op("['a','b','c','d','e'].join(',')", 2).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  ['a','b','c','d','e'].join(',')\n", "  ~~~~~~~~~~~~~~~~~~~~~~^~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn contains_iterations_count() {
    let e = eval_op("99 in range(1000)", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  99 in range(1000)\n", "        ^~~~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn list_concat_iterations_count() {
    let e = eval_op("range(500) + range(500)", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  range(500) + range(500)\n", "  ^~~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn list_multiply_iterations_count() {
    let e = eval_op("[1, 2, 3] * 1000", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  [1, 2, 3] * 1000\n", "  ~~~~~~~~~~^~~~~~"].concat()), "got:\n{e}");
}

#[test] fn any_iterations_count() {
    let e = eval_op("any([False] * 1000)", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  any([False] * 1000)\n", "      ~~~~~~~~^~~~~~"].concat()), "got:\n{e}");
}

#[test] fn all_iterations_count() {
    let e = eval_op("all([True] * 1000)", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  all([True] * 1000)\n", "      ~~~~~~~^~~~~~"].concat()), "got:\n{e}");
}

#[test] fn flatten_iterations_count() {
    let e = eval_op("flatten([[1,2],[3,4]] * 500)", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  flatten([[1,2],[3,4]] * 500)\n", "          ~~~~~~~~~~~~~~^~~~~"].concat()), "got:\n{e}");
}

#[test] fn repr_sh_list_iterations_count() {
    let e = eval_op("repr_sh(range(1000))", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
        "  repr_sh(range(1000))\n", "          ^~~~~~~~~~~"].concat()), "got:\n{e}");
}

#[test] fn list_equality_iterations_count() {
    let e = eval_op("range(1000) == range(1000)", 100).unwrap_err().to_string();
    assert!(e.contains(&["Operation limit exceeded\n",
...
#[test] fn simple_arithmetic_within_limit() {
...
#[test] fn small_comprehension_within_limit() {
    assert!(eval_op("[x * 2 for x in range(5)]", 1000).is_ok());
}

#[test] fn default_limit_handles_normal() {
    let r = evaluate_expression("sum(range(100))", &SymbolTable::new()).unwrap();
    assert_eq!(r.to_display_string(), "4950");
}

#[test] fn string_operations_within_limit() {
    assert_eq!(eval_op("'hello'.upper()", 100).unwrap().value.to_display_string(), "HELLO");
}

#[test] fn chained_operations_within_limit() {
    assert!(eval_op("'a,b,c'.split(',').join(';')", 100).is_ok());
}

// === TestOperationCount ===

#[test] fn operation_count_returned() {
    assert!(op_count("1 + 2") > 0);
}

#[test] fn constant_has_zero_operations() {
    assert_eq!(op_count("42"), 0);
}

#[test] fn single_function_call_is_one_operation() {
    assert_eq!(op_count("1 + 2"), 1);
}

#[test] fn range_counts_call_plus_iterations() {
    // 1 call + 10 iterations = 11
    assert_eq!(op_count("range(10)"), 11);
}

#[test] fn sum_range_counts_both() {
...
#[test] fn nested_comprehension_accumulates() {
    // range: 1 call + 10 iterations = 11
...
#[test] fn constant_has_zero_operations_v2() {
    let r = evaluate_expression_bounded("42", &SymbolTable::new(), 100_000_000, 10_000_000).unwrap();
    assert_eq!(r.operation_count, 0);
}
#[test] fn single_function_call_is_one_v2() { let r = evaluate_expression_bounded("abs(-5)", &SymbolTable::new(), 100_000_000, 10_000_000).unwrap(); assert!(r.operation_count >= 1); }
#[test] fn range_counts_call_plus_iterations_v2() {
    let r = evaluate_expression_bounded("range(5)", &SymbolTable::new(), 100_000_000, 10_000_000).unwrap();
...
#[test] fn string_slice_charges_string_ops() {
