...
pub fn parse(path: &str) -> Option<UriParts> {
    ...
    Some(UriParts { authority, path_parts })
}
...
pub fn parent(path: &str) -> String {
    let Some(uri) = parse(path) else { return path.to_string() };
...
pub fn suffix(path: &str) -> String {
    let n = name(path);
    n.rfind('.').filter(|&i| i > 0).map(|i| n[i..].to_string()).unwrap_or_default()
}
...
pub fn stem(path: &str) -> String {
    let n = name(path);
    n.rfind('.').filter(|&i| i > 0).map(|i| n[..i].to_string()).unwrap_or(n)
}

/// Split into parts: first element is `scheme://authority`, rest are path segments.
pub fn parts(path: &str) -> Vec<String> {
    let Some(uri) = parse(path) else { return vec![path.to_string()] };
...
pub fn join(path: &str, child: &str) -> String {
    let Some(uri) = parse(path) else { return format!("{path}/{child}") };
    let mut p = uri.path_parts;
    // Remove trailing empty part (from trailing slash) before appending
    if p.last().map_or(false, |s| s.is_empty()) {
        p.pop();
    }
...
mod tests {
    ...
    #[test] fn not_uri() { assert!(!is_uri("/local/path")); }
    #[test] fn not_uri_windows() { assert!(!is_uri("C:\\path")); }
    #[test] fn s3_is_uri() { assert!(is_uri("s3://bucket/key")); }
    #[test] fn https_is_uri() { assert!(is_uri("https://host/path")); }
    ...
    #[test] fn name_basic() { assert_eq!(name("s3://bucket/dir/file.txt"), "file.txt"); }
    #[test] fn name_bare() { assert_eq!(name("s3://bucket"), ""); }
    #[test] fn name_trailing_slash() { assert_eq!(name("s3://bucket/dir/"), ""); }

    #[test] fn parent_basic() { assert_eq!(parent("s3://bucket/dir/file.txt"), "s3://bucket/dir"); }
    #[test] fn parent_single() { assert_eq!(parent("s3://bucket/file.txt"), "s3://bucket"); }
    #[test] fn parent_bare() { assert_eq!(parent("s3://bucket"), "s3://bucket"); }

    #[test] fn suffix_basic() { assert_eq!(suffix("s3://bucket/file.tar.gz"), ".gz"); }
    #[test] fn suffix_none() { assert_eq!(suffix("s3://bucket/file"), ""); }

    #[test] fn suffixes_compound() { assert_eq!(suffixes("s3://bucket/file.tar.gz"), vec![".tar", ".gz"]); }
    #[test] fn suffixes_none() { assert_eq!(suffixes("s3://bucket/file"), Vec::<String>::new()); }

    #[test] fn stem_basic() { assert_eq!(stem("s3://bucket/file.tar.gz"), "file.tar"); }
    #[test] fn stem_no_ext() { assert_eq!(stem("s3://bucket/file"), "file"); }

    #[test] fn parts_basic() { assert_eq!(parts("s3://bucket/dir/file"), vec!["s3://bucket", "dir", "file"]); }
    #[test] fn parts_bare() { assert_eq!(parts("s3://bucket"), vec!["s3://bucket"]); }

    #[test] fn from_parts_basic() { assert_eq!(from_parts(&["s3://bucket".into(), "dir".into(), "file".into()]), "s3://bucket/dir/file"); }
    #[test] fn from_parts_bare() { assert_eq!(from_parts(&["s3://bucket".into()]), "s3://bucket"); }
}
