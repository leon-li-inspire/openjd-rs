...
#[test]
fn int_list() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": [1, 2, 3]}]}"#));
}

#[test]
fn int_min_len_list() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": [1]}]}"#));
}
...
#[test]
fn int_mixed_types() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": ["1", 2]}]}"#));
}
...
fn int_range_expression() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": "1-10"}]}"#));
}

#[test]
fn int_range_expr_with_step() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": "1-10:2"}]}"#));
}

#[test]
fn int_range_expr_negative() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": "10--5:-1"}]}"#));
}

#[test]
fn int_range_expr_two_ranges() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": "-10-0,1-10"}]}"#));
}
...
#[test]
fn float_list() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "FLOAT", "range": [1.0, 2.5]}]}"#));
}

#[test]
fn float_int_values() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "FLOAT", "range": [1, 2]}]}"#));
}

#[test]
fn float_as_string() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "FLOAT", "range": ["1.1"]}]}"#));
}

#[test]
fn float_mixed_types() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "FLOAT", "range": ["1", 2, 3.3, "3.4"]}]}"#));
}

#[test]
fn float_format_string() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "FLOAT", "range": ["{{Param.Value}}"]}]}"#));
}
...
#[test]
fn string_list() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "STRING", "range": ["a", "b"]}]}"#));
}
...
#[test]
fn path_list() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "PATH", "range": ["/tmp/a", "/tmp/b"]}]}"#));
}
...
fn param_space_float() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "FLOAT", "range": [1]}]}"#));
}

#[test]
fn param_space_string() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "STRING", "range": ["1"]}]}"#));
}

#[test]
fn param_space_path() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "PATH", "range": ["/tmp"]}]}"#));
}

#[test]
fn param_space_max_params() {
    let params: Vec<String> = (0..16).map(|i| format!(r#"{{"name": "foo{i}", "type": "INT", "range": [1]}}"#)).collect();
    decode_ok(&job_with_param_space(&format!(r#"{{"taskParameterDefinitions": [{}]}}"#, params.join(","))));
}

#[test]
fn combination_product() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "A", "type": "INT", "range": [1, 2]}, {"name": "B", "type": "INT", "range": [3, 4]}], "combination": "A * B"}"#));
}
...
fn combination_product_assoc() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "A", "type": "INT", "range": [1]}, {"name": "B", "type": "INT", "range": [1]}, {"name": "C", "type": "INT", "range": [1]}], "combination": "A * (B, C)"}"#));
}
...
fn combination_nested_product_in_assoc() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "A", "type": "INT", "range": [1]}, {"name": "B", "type": "INT", "range": [1]}, {"name": "C", "type": "INT", "range": [1]}, {"name": "D", "type": "INT", "range": [1]}], "combination": "(A * B, C * D)"}"#));
}
...
fn combination_multi_product() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "A", "type": "INT", "range": [1]}, {"name": "B", "type": "INT", "range": [1]}, {"name": "C", "type": "INT", "range": [1]}, {"name": "D", "type": "INT", "range": [1]}, {"name": "E", "type": "INT", "range": [1]}], "combination": "A * B * C * D * E"}"#));
}
...
fn empty_task_params() {
    check_err(&job_with_param_space(r#"{"taskParameterDefinitions": []}"#), &[
...
fn too_many_task_params() {
    let params: Vec<String> = (0..17).map(|i| format!(r#"{{"name": "foo{i}", "type": "INT", "range": [1]}}"#)).collect();
    check_err(&job_with_param_space(&format!(r#"{{"taskParameterDefinitions": [{}]}}"#, params.join(","))), &[
        "steps[0] -> parameterSpace -> taskParameterDefinitions:\n\texceeds 16 elements.",
    ]);
}
...
fn combination_empty() {
    check_err(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "A", "type": "INT", "range": [1]}], "combination": ""}"#), &[
...
fn combination_missing_operator() {
    check_err(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": [1]}, {"name": "bar", "type": "INT", "range": [1]}], "combination": "foo  bar"}"#), &[
...
fn combination_unclosed_paren() {
    check_err(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "A", "type": "INT", "range": [1]}, {"name": "B", "type": "INT", "range": [1]}], "combination": "(A, B"}"#), &[
        "steps[0] -> parameterSpace -> combination:\n\tunmatched '('.",
    ]);
}
...
fn combination_missing_comma_in_assoc() {
    check_err(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "A", "type": "INT", "range": [1]}, {"name": "B", "type": "INT", "range": [1]}, {"name": "C", "type": "INT", "range": [1]}], "combination": "(A, B C)"}"#), &[
        "steps[0] -> parameterSpace -> combination:\n\tmissing operator between parameters.",
    ]);
}
...
fn combination_unknown_param() {
    check_err(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": [1]}, {"name": "bar", "type": "INT", "range": [1]}], "combination": "foo * bar * baz"}"#), &[
...
fn int_max_len_list() {
    ...
    decode_ok(&job_with_param_space(&format!(r#"{{"taskParameterDefinitions": [{{"name": "foo", "type": "INT", "range": [{items}]}}]}}"#)));
}
...
fn float_max_len_list() {
    ...
    decode_ok(&job_with_param_space(&format!(r#"{{"taskParameterDefinitions": [{{"name": "foo", "type": "FLOAT", "range": [{items}]}}]}}"#)));
}

#[test]
fn string_min_len_list() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "STRING", "range": ["a"]}]}"#));
}

#[test]
fn string_max_len_list() {
    let items: Vec<String> = (0..1024).map(|i| format!("\"s{i}\"")).collect();
    decode_ok(&job_with_param_space(&format!(r#"{{"taskParameterDefinitions": [{{"name": "foo", "type": "STRING", "range": [{}]}}]}}"#, items.join(","))));
}
...
fn path_max_len_list() {
    ...
    decode_ok(&job_with_param_space(&format!(r#"{{"taskParameterDefinitions": [{{"name": "foo", "type": "PATH", "range": [{}]}}]}}"#, items.join(","))));
}

// Range expression additional success cases
#[test]
fn range_expr_negative_range_with_negative_steps() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": "-5--14:-2"}]}"#));
}

#[test]
fn range_expr_two_ranges_opposite_signs() {
    decode_ok(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": "10-1:-1,11-20:2"}]}"#));
}

// INT failure: disallow floats
#[test]
fn int_disallow_floats() {
    check_err(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": [1.1]}]}"#), &[
...
fn int_disallow_bool() {
    check_err(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": [true]}]}"#), &[
...
fn int_literal_string_not_int() {
    check_err(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "foo", "type": "INT", "range": ["notint"]}]}"#), &[
...
fn combination_single_element_association() {
    ...
    let result = decode_job_template(
        yaml_val(&job_with_param_space(r#"{"taskParameterDefinitions": [{"name": "A", "type": "INT", "range": [1]}], "combination": "(A)"}"#)),
        None,
    );
    // Either it fails with an error about association, or it succeeds (treating (A) as A)
    // Both are acceptable behaviors
    if let Err(e) = result {
        assert!(e.to_string().contains("combination") || e.to_string().contains("association"),
            "Expected combination error, got: {}", e);
    }
}
