...
fn build_param_symtab(jt: &JobTemplate) -> SymbolTable {
    ...
    if let Some(params) = &jt.parameter_definitions {
        for p in params {
            ...
            symtab.set(&format!("Param.{}", p.name()), ExprValue::unresolved(expr_type.clone())).expect("symtab");
            let raw_type = match pt {
                JobParameterType::Path => ExprType::STRING,
                JobParameterType::ListPath => ExprType::list(ExprType::STRING),
                _ => expr_type,
            };
            symtab.set(&format!("RawParam.{}", p.name()), ExprValue::unresolved(raw_type)).expect("symtab");
        }
    }
    symtab
}
...
fn build_session_scope_symtab(jt: &JobTemplate, env: &Environment, is_step_env: bool, expr_active: bool) -> SymbolTable {
    let mut symtab = build_param_symtab(jt);
    symtab.set("Session.WorkingDirectory", ExprValue::unresolved(ExprType::PATH)).expect("symtab");
    symtab.set("Session.HasPathMappingRules", ExprValue::unresolved(ExprType::BOOL)).expect("symtab");
    symtab.set("Session.PathMappingRulesFile", ExprValue::unresolved(ExprType::PATH)).expect("symtab");
    // Step.Name available in step environments with EXPR
    if is_step_env && expr_active {
        symtab.set("Step.Name", ExprValue::unresolved(ExprType::STRING)).expect("symtab");
    }
    // Job.Name available in all environments with EXPR
    if expr_active {
        symtab.set("Job.Name", ExprValue::unresolved(ExprType::STRING)).expect("symtab");
    }
    // Env.File.* from this environment's script
    if let Some(script) = &env.script {
        if let Some(files) = &script.embedded_files {
            for f in files {
                symtab.set(&format!("Env.File.{}", f.name), ExprValue::unresolved(ExprType::PATH)).expect("symtab");
            }
        }
    }
    symtab
...
fn build_task_scope_symtab(
    ...
    if let Some(ps) = &step.parameter_space {
        for tp in &ps.task_parameter_definitions {
            ...
            symtab.set(&format!("Task.Param.{}", tp.name()), ExprValue::unresolved(tp_type.clone())).expect("symtab");
            ...
            symtab.set(&format!("Task.RawParam.{}", tp.name()), ExprValue::unresolved(raw_type)).expect("symtab");
        }
    }

    // Task.File.* from step script embedded files
    if let Some(script) = step.resolve_syntax_sugar().as_ref().or(step.script.as_ref()) {
        if let Some(files) = &script.embedded_files {
            for f in files {
                symtab.set(&format!("Task.File.{}", f.name), ExprValue::unresolved(ExprType::PATH)).expect("symtab");
            }
        }
    }
...
fn validate_fs(fs: &FormatString, symtab: &SymbolTable, lib: &FunctionLibrary, path: &[PathElement], errors: &mut ValidationErrors) {
    if fs.is_literal() { return; }
...
fn validate_action_fs(action: &Action, symtab: &SymbolTable, lib: &FunctionLibrary, action_path: &[PathElement], errors: &mut ValidationErrors) {
    validate_fs(&action.command, symtab, lib, &path_field(action_path, "command"), errors);
...
pub fn validate_format_strings(jt: &JobTemplate, ctx: &ValidationContext, errors: &mut ValidationErrors) {
    ...
    validate_fs(&jt.name, &template_symtab, &default_lib, &path_field(&[], "name"), errors);
    ...
    for (i, step) in jt.steps.iter().enumerate() {
        ...
        if let Some(hr) = &step.host_requirements {
            ...
            if expr_active {
                hr_symtab.set("Job.Name", ExprValue::unresolved(ExprType::STRING)).expect("symtab");
                hr_symtab.set("Step.Name", ExprValue::unresolved(ExprType::STRING)).expect("symtab");
                if let Some(bindings) = &step.let_bindings {
                    let let_path = path_field(&step_path, "let");
                    let mut hr_let_names = HashSet::new();
                    validate_let_bindings(bindings, &let_path, &HashSet::new(), &mut hr_let_names, &mut hr_symtab, &default_lib, errors);
                }
            }
            ...
            if let Some(amounts) = &hr.amounts {
                for (j, amt) in amounts.iter().enumerate() {
                    ...
                    if let Some(min) = &amt.min { validate_fs(min, &hr_symtab, &default_lib, &path_field(&amt_path, "min"), errors); }
                    if let Some(max) = &amt.max { validate_fs(max, &hr_symtab, &default_lib, &path_field(&amt_path, "max"), errors); }
                }
            }
            if let Some(attrs) = &hr.attributes {
                for (j, attr) in attrs.iter().enumerate() {
                    ...
                    if let Some(any_of) = &attr.any_of {
                        for (k, v) in any_of.iter().enumerate() { validate_fs(v, &hr_symtab, &default_lib, &path_index(&path_field(&attr_path, "anyOf"), k), errors); }
                    }
                    if let Some(all_of) = &attr.all_of {
                        for (k, v) in all_of.iter().enumerate() { validate_fs(v, &hr_symtab, &default_lib, &path_index(&path_field(&attr_path, "allOf"), k), errors); }
                    }
                }
            }
        }
            ...
            if expr_active {
                if let Some(script) = &env.script {
                    if let Some(bindings) = &script.let_bindings {
                        ...
                        validate_let_bindings(bindings, &let_path, &HashSet::new(), &mut env_let_names, &mut env_symtab, &host_lib, errors);
                    }
                }
            }
            validate_env_format_strings(env, &env_symtab, &host_lib, &path_index(&envs_path, i), errors);
        }
    }
    ...
    for (i, step) in jt.steps.iter().enumerate() {
        ...
        if let Some(ps) = &step.parameter_space {
            ...
            if expr_active {
                range_symtab.set("Job.Name", ExprValue::unresolved(ExprType::STRING)).expect("symtab");
                range_symtab.set("Step.Name", ExprValue::unresolved(ExprType::STRING)).expect("symtab");
                    ...
                    for binding in bindings {
                        if let Some(eq_pos) = binding.find('=') {
                            ...
                            if !name.is_empty() && !expr_str.is_empty() {
                                if let Ok(parsed) = openjd_expr::eval::ParsedExpression::new(expr_str) {
                ...
                match tp {
                    TaskParameterDefinition::INT(t) => {
                        if let crate::template::IntRange::Expression(expr) = &t.range {
                            validate_fs(expr, &range_symtab, &default_lib, &path_field(&p_path, "range"), errors);
                        }
                    }
                    TaskParameterDefinition::STRING(t) => {
                        if let crate::template::StringRange::List(items) = &t.range {
                            for (k, item) in items.iter().enumerate() {
                                validate_fs(item, &range_symtab, &default_lib, &path_index(&path_field(&p_path, "range"), k), errors);
                            }
                        }
                    }
        ...
        if let Some(bindings) = &step.let_bindings {
            ...
            if !expr_active {
                ...
                step_let_symtab.set("Job.Name", ExprValue::unresolved(ExprType::STRING)).expect("symtab");
                step_let_symtab.set("Step.Name", ExprValue::unresolved(ExprType::STRING)).expect("symtab");
                let mut step_let_names = HashSet::new();
                validate_let_bindings(bindings, &let_path, &HashSet::new(), &mut step_let_names, &mut step_let_symtab, &default_lib, errors);
        ...
        if let Some(script) = &step.script {
            ...
            if let Some(bindings) = &script.let_bindings {
                ...
                if !expr_active {
                    ...
                    let enclosing: HashSet<String> = step.let_bindings.as_ref()
                        .map(|bs| bs.iter().filter_map(|b| b.find('=').map(|eq| b[..eq].trim().to_string())).collect())
                        .unwrap_or_default();
                    let mut script_let_names = HashSet::new();
                    validate_let_bindings(bindings, &let_path, &enclosing, &mut script_let_names, &mut task_symtab, &host_lib, errors);
                }
            }
            ...
            if !expr_active {
                ...
                if script.actions.on_run.command.has_complex_expressions() {
                    errors.add(&path_field(&action_path, "command"), "complex expressions require the EXPR extension.");
                }
                if let Some(args) = &script.actions.on_run.args {
                    let args_path = path_field(&action_path, "args");
                    for (j, arg) in args.iter().enumerate() {
                        if arg.has_complex_expressions() {
                            errors.add(&path_index(&args_path, j), "complex expressions require the EXPR extension.");
                        }
                    }
                }
            }
            ...
            let action_path = path_field(&path_field(&script_path, "actions"), "onRun");
            validate_action_fs(&script.actions.on_run, &task_symtab, &host_lib, &action_path, errors);

            // Timeout
            if let Some(timeout) = &script.actions.on_run.timeout {
                validate_fs(timeout, &task_symtab, &host_lib, &path_field(&action_path, "timeout"), errors);
            }
            if let Some(cancel) = &script.actions.on_run.cancelation {
                if let CancelationMode::NotifyThenTerminate { notify_period_in_seconds: Some(notify) } = cancel {
                    validate_fs(notify, &task_symtab, &host_lib, &path_field(&action_path, "cancelation"), errors);
                }
            }
            ...
            if let Some(files) = &script.embedded_files {
                ...
                for (j, f) in files.iter().enumerate() {
                    ...
                    if let Some(data) = &f.data { validate_fs(data, &task_symtab, &host_lib, &path_field(&f_path, "data"), errors); }
                    if let Some(filename) = &f.filename { validate_fs(filename, &task_symtab, &host_lib, &path_field(&f_path, "filename"), errors); }
                }
            }
            ...
            if expr_active {
                ...
                if let Some(bindings) = &step.let_bindings {
                    for b in bindings {
                        if let Some(eq) = b.find('=') { all_let_names.insert(b[..eq].trim().to_string()); }
                    }
                }
                if let Some(bindings) = &script.let_bindings {
                    for b in bindings {
                ...
                if !all_let_names.is_empty() {
                    if let Err(e) = script.actions.on_run.command.validate_comprehension_vars(&all_let_names) {
        ...
        if let Some(envs) = &step.step_environments {
            ...
            for (j, env) in envs.iter().enumerate() {
                    ...
                    if let Some(bindings) = &step.let_bindings {
                        for b in bindings {
                            if let Some(eq) = b.find('=') {
                                let name = b[..eq].trim();
                                if !name.is_empty() {
                                    if let Some(openjd_expr::symbol_table::SymbolTableEntry::Value(val)) = task_symtab.get(name) {
                ...
                if let Some(script) = &env.script {
                    if let Some(bindings) = &script.let_bindings {
                        if expr_active {
                            let env_let_path = path_field(&path_field(&path_index(&envs_path, j), "script"), "let");
                            let enclosing: HashSet<String> = step.let_bindings.as_ref()
                                .map(|bs| bs.iter().filter_map(|b| b.find('=').map(|eq| b[..eq].trim().to_string())).collect())
                                .unwrap_or_default();
                            let mut env_let_names = HashSet::new();
                            validate_let_bindings(bindings, &env_let_path, &enclosing, &mut env_let_names, &mut env_symtab, &host_lib, errors);
                        }
                    }
                }
                validate_env_format_strings(env, &env_symtab, &host_lib, &path_index(&envs_path, j), errors);
            }
        }

        // SimpleAction let bindings (requires both FB1 and EXPR)
        if expr_active {
            for sa_field in [&step.bash, &step.python, &step.cmd, &step.powershell, &step.node] {
                if let Some(sa) = sa_field {
                    let mut sa_let_names: HashSet<String> = HashSet::new();
                    if let Some(bindings) = &step.let_bindings {
                        for b in bindings {
                            if let Some(eq) = b.find('=') { sa_let_names.insert(b[..eq].trim().to_string()); }
                        }
                    }
                    if let Some(let_bindings) = &sa.let_bindings {
                        if ctx.has_extension(KnownExtension::FeatureBundle1) {
                            ...
                            validate_let_bindings(let_bindings, &step_path, &enclosing, &mut new_names, &mut task_symtab, &host_lib, errors);
...
fn validate_env_format_strings(env: &Environment, symtab: &SymbolTable, lib: &FunctionLibrary, path: &[PathElement], errors: &mut ValidationErrors) {
    ...
    if let Some(script) = &env.script {
        ...
        if let Some(action) = &script.actions.on_enter {
            validate_action_fs(action, symtab, lib, &path_field(&actions_path, "onEnter"), errors);
        }
        if let Some(action) = &script.actions.on_exit {
            validate_action_fs(action, symtab, lib, &path_field(&actions_path, "onExit"), errors);
        }
        if let Some(files) = &script.embedded_files {
            let files_path = path_field(&script_path, "embeddedFiles");
            for (j, f) in files.iter().enumerate() {
                let f_path = path_index(&files_path, j);
                if let Some(data) = &f.data { validate_fs(data, symtab, lib, &path_field(&f_path, "data"), errors); }
                if let Some(filename) = &f.filename { validate_fs(filename, symtab, lib, &path_field(&f_path, "filename"), errors); }
            }
        }
    }
}
    ...
    if let Some(envs) = envs {
        for env in envs {
            if let Some(script) = &env.script {
                ...
                if let Some(bindings) = &script.let_bindings {
                    for b in bindings {
                        if let Some(eq) = b.find('=') { env_let_names.insert(b[..eq].trim().to_string()); }
                    }
                }
                if !env_let_names.is_empty() {
                    let path: Vec<PathElement> = vec![];
                    for action in [&script.actions.on_enter, &script.actions.on_exit].into_iter().flatten() {
...
fn validate_let_bindings(
    ...
    for (i, binding) in bindings.iter().enumerate() {
        ...
        if expr.is_empty() {
            errors.add(&b_path, format!("binding '{name}' has no expression after '='."));
            continue;
        }
        let first = name.chars().next().unwrap();
        if !first.is_ascii_lowercase() && first != '_' {
            errors.add(&b_path, format!("name '{name}' must start with lowercase letter or underscore."));
        }
        if !name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
            errors.add(&b_path, format!("name '{name}' contains invalid characters."));
        }
        ...
        if expr.contains(name) {
            ...
            static STRING_LITERAL_RE: LazyLock<regex::Regex> = LazyLock::new(|| regex::Regex::new(r#""[^"]*"|'[^']*'"#).unwrap());
        ...
        let expr_start = eq_pos + 1 + binding[eq_pos + 1..].len() - binding[eq_pos + 1..].trim_start().len();
        ...
        match ParsedExpression::new(expr) {
            Ok(parsed) => {
                ...
                match evaluator.evaluate(&parsed.ast) {
                    ...
                    Err(e) => {
                        errors.add(&b_path, format!(
                            "Invalid expression in let binding '{name}': {}", e.message_with_expr_prefix(prefix)
                        ));
