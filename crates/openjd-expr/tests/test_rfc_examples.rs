...
// Basic RFC examples
#[test] fn rfc_arithmetic() { assert_eq!(eval("1 + 2 * 3").to_display_string(), "7"); }
#[test] fn rfc_string_concat() { assert_eq!(eval("'hello' + ' ' + 'world'").to_display_string(), "hello world"); }
#[test] fn rfc_conditional() { assert_eq!(eval("'yes' if True else 'no'").to_display_string(), "yes"); }
#[test] fn rfc_list_comp() { assert_eq!(eval("[x * 2 for x in [1, 2, 3]]").to_display_string(), "[2, 4, 6]"); }
...
fn rfc_string_formatting() {
    let mut st = SymbolTable::new();
    st.set("Param.Frame", ExprValue::Int(42)).unwrap();
    assert_eq!(eval_with("zfill(Param.Frame, 4)", &st).to_display_string(), "0042");
}

// === Additional RFC examples ===
#[test] fn rfc_string_manipulation() { assert_eq!(eval("'hello world'.upper()").to_display_string(), "HELLO WORLD"); }
#[test] fn rfc_path_with_suffix() {
    let mut st = SymbolTable::new();
    st.set("P", ExprValue::Path { value: "/renders/scene.exr".into(), format: PathFormat::Posix }).unwrap();
    assert_eq!(evaluate_expression("P.with_suffix('.png')", &st).unwrap().to_display_string(), "/renders/scene.png");
}
#[test] fn rfc_repr_sh_string() { assert_eq!(eval("repr_sh('hello world')").to_display_string(), "'hello world'"); }
#[test] fn rfc_repr_sh_list() { assert_eq!(eval("repr_sh([1, 2, 3])").to_display_string(), "1 2 3"); }
#[test] fn rfc_gpu_flag_true() {
    let mut st = SymbolTable::new();
...
fn rfc_frame_range_arithmetic() {
    ...
    let r = eval_with("min(Task.Param.Frame + Param.FramesPerTask, Param.FrameEnd) - 1", &st);
...
fn rfc_conditional_draft() {
    let mut st = SymbolTable::new();
    st.set("Param.Quality", ExprValue::String("draft".into())).unwrap();
...
fn rfc_conditional_final() {
    let mut st = SymbolTable::new();
    st.set("Param.Quality", ExprValue::String("final".into())).unwrap();
    let r = eval_with("16 if Param.Quality == 'final' else 4", &st);
...
fn rfc_quality_list_with_value() {
    ...
    let r = eval_with("['--quality', string(Param.Quality)] if Param.Quality > 0 else null", &st);
...
fn rfc_string_manipulation_path() {
    let mut st = SymbolTable::new();
    st.set("Param.InputFile", ExprValue::Path {
...
fn rfc_path_division_with_suffix() {
    ...
    st.set("Param.OutputDir", ExprValue::Path {
        value: "/output".into(),
        format: PathFormat::Posix,
    }).unwrap();
    let r = eval_with_path_format(
