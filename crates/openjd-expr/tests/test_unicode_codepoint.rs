...
#[test] fn len_cafe() { assert_eq!(eval("len('café')"), ExprValue::Int(4)); }
#[test] fn len_nihongo() { assert_eq!(eval("len('日本語')"), ExprValue::Int(3)); }
#[test] fn len_emoji() { assert_eq!(eval("len('hello🌍')"), ExprValue::Int(6)); }
#[test] fn len_zwj_family() {
...
#[test] fn center_cjk() {
    ...
    assert_eq!(eval("center('日本語', 9)").to_display_string(), "   日本語   ");
}
#[test] fn center_emoji() {
    // Python: "🌍".center(5) == "  🌍  "
    // 1 codepoint, width 5 → 4 padding (2 left, 2 right)
    assert_eq!(eval("center('🌍', 5)").to_display_string(), "  🌍  ");
}
#[test] fn center_already_wide() {
...
#[test] fn ljust_cjk() {
    ...
    assert_eq!(eval("ljust('日本語', 9)").to_display_string(), "日本語      ");
}
#[test] fn ljust_emoji() {
    // Python: "🌍".ljust(5) == "🌍    "
...
#[test] fn rjust_cjk() {
    ...
    assert_eq!(eval("rjust('日本語', 9)").to_display_string(), "      日本語");
}
#[test] fn rjust_emoji() {
    // Python: "🌍".rjust(5) == "    🌍"
...
#[test] fn repr_json_newline() {
    // Python: json.dumps("hello\nworld") == '"hello\\nworld"'
    assert_eq!(eval(r#"repr_json("hello\nworld")"#).to_display_string(), r#""hello\nworld""#);
}
#[test] fn repr_json_tab() {
    // Python: json.dumps("hello\tworld") == '"hello\\tworld"'
    assert_eq!(eval(r#"repr_json("hello\tworld")"#).to_display_string(), r#""hello\tworld""#);
}
#[test] fn repr_json_carriage_return() {
    // Python: json.dumps("hello\rworld") == '"hello\\rworld"'
    assert_eq!(eval(r#"repr_json("hello\rworld")"#).to_display_string(), r#""hello\rworld""#);
}
#[test] fn repr_json_ensure_ascii_cafe() {
    // Python: json.dumps("café") == '"caf\\u00e9"'
    assert_eq!(eval("repr_json('café')").to_display_string(), r#""caf\u00e9""#);
}
#[test] fn repr_json_ensure_ascii_cjk() {
    // Python: json.dumps("日本") == '"\\u65e5\\u672c"'
    assert_eq!(eval("repr_json('日本')").to_display_string(), r#""\u65e5\u672c""#);
}
#[test] fn repr_json_ensure_ascii_emoji() {
    // Python: json.dumps("🌍") == '"\\ud83c\\udf0d"' (surrogate pair)
    assert_eq!(eval("repr_json('🌍')").to_display_string(), r#""\ud83c\udf0d""#);
}
...
#[test] fn find_after_zwj_emoji() {
    // "a" + ZWJ family (5 codepoints) + "b" = 7 codepoints total
    // Python: "a👨\u200d👩\u200d👧b".find("b") == 6
    assert_eq!(eval("find('a👨\u{200D}👩\u{200D}👧b', 'b')"), ExprValue::Int(6));
}
#[test] fn len_string_with_zwj() {
    // Python: len("a👨\u200d👩\u200d👧b") == 7
