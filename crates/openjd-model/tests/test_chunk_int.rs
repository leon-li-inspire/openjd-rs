...
fn decode_ok(s: &str) {
    let v = yaml_val(s);
    decode_job_template(v, Some(&["TASK_CHUNKING"])).expect(&format!("Expected success for: {s}"));
}
...
#[test]
fn min_len_int_list() {
    decode_ok(&chunk_job(r#"{"name": "foo", "type": "CHUNK[INT]", "range": [1], "chunks": {"defaultTaskCount": 1, "rangeConstraint": "CONTIGUOUS"}}"#));
}

#[test]
fn range_expression() {
    decode_ok(&chunk_job(r#"{"name": "foo", "type": "CHUNK[INT]", "range": "1-100", "chunks": {"defaultTaskCount": 10, "rangeConstraint": "NONCONTIGUOUS"}}"#));
}
...
#[test]
fn mixed_int_types() {
    decode_ok(&chunk_job(r#"{"name": "foo", "type": "CHUNK[INT]", "range": ["1", 2], "chunks": {"defaultTaskCount": 1, "rangeConstraint": "CONTIGUOUS"}}"#));
}

#[test]
fn target_runtime_seconds_zero() {
    decode_ok(&chunk_job(r#"{"name": "foo", "type": "CHUNK[INT]", "range": "1-100", "chunks": {"defaultTaskCount": 10, "targetRuntimeSeconds": 0, "rangeConstraint": "NONCONTIGUOUS"}}"#));
}

#[test]
fn target_runtime_seconds_1000() {
    decode_ok(&chunk_job(r#"{"name": "foo", "type": "CHUNK[INT]", "range": "1-100", "chunks": {"defaultTaskCount": 10, "targetRuntimeSeconds": 1000, "rangeConstraint": "NONCONTIGUOUS"}}"#));
}

#[test]
fn default_task_count_is_str() {
    decode_ok(&chunk_job(r#"{"name": "foo", "type": "CHUNK[INT]", "range": "1-100", "chunks": {"defaultTaskCount": "10", "targetRuntimeSeconds": 100, "rangeConstraint": "NONCONTIGUOUS"}}"#));
}
...
fn requires_task_chunking_extension() {
    check_err_no_ext(&chunk_job_no_ext(
...
fn missing_chunks_field() {
    check_err(&chunk_job(
        r#"{"name": "foo", "type": "CHUNK[INT]", "range": [1]}"#
    ), &[
...
use openjd_model::{preprocess_job_parameters, create_job};
...
fn create_chunked_job(template_json: &str) -> openjd_model::job::Job {
    ...
    let supported = vec!["TASK_CHUNKING"];
...
fn chunked_contiguous_noncontig_range() {
    ...
    assert!(tasks.len() >= 1 && tasks.len() <= 3, "Expected 1-3 chunks, got {}", tasks.len());
}
