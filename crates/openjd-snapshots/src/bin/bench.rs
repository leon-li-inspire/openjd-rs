...
#[derive(Parser)]
#[command(name = "snapshots-bench", about = "Benchmark openjd-snapshots operations")]
struct Cli {
...
fn create_test_files(root: &Path, cli: &Cli) -> (usize, u64, HashMap<String, String>) {
    ...
    if cli.small_files > 0 && !subdirs.is_empty() {
        eprintln!("  Creating {} small files ({} bytes each)...", cli.small_files, SMALL_FILE_SIZE);
        ...
        let warm_dir = if subdirs.len() > 1 { &subdirs[1] } else { &subdirs[0] };
        let other_dirs = if subdirs.len() > 2 { &subdirs[2..] } else { &subdirs[..1] };
    ...
    if cli.medium_files > 0 {
        ...
        eprintln!("  Creating {} medium files ({}MB each)...", cli.medium_files, MEDIUM_FILE_SIZE / (1024 * 1024));
    ...
    if cli.large_files > 0 {
        ...
        eprintln!("  Creating {} large files ({}GB each)...", cli.large_files, large_size / (1024 * 1024 * 1024));
    ...
    eprintln!("  Total: {} files, {:.2} MB", total_files, total_bytes as f64 / (1024.0 * 1024.0));
    (total_files, total_bytes, checksums)
}

fn format_duration(secs: f64) -> String {
    if secs < 1.0 { format!("0:{:04.1}", secs) }
    else { format!("{}:{:02.0}", (secs / 60.0) as u64, secs % 60.0) }
}
...
fn main() {
    ...
    match cli.preset.as_deref() {
        Some("tiny") => { cli.small_files = 400; cli.medium_files = 40; cli.large_files = 2; }
        Some("small") => { cli.small_files = 1500; cli.medium_files = 400; cli.large_files = 5; }
        Some("medium") => { cli.small_files = 20000; cli.medium_files = 1000; cli.large_files = 10; }
        Some("large") => { cli.small_files = 1_000_000; cli.medium_files = 10000; cli.large_files = 10; }
        Some(p) => { eprintln!("Unknown preset: {}", p); std::process::exit(1); }
        None => {}
    }

    let worker_counts: Vec<usize> = cli.max_workers.split(',')
        .map(|s| s.trim().parse().expect("invalid worker count"))
        .collect();
    let is_scaling = worker_counts.len() > 1;

    let chunk_size_bytes: i64 = if cli.no_chunking { WHOLE_FILE_CHUNK_SIZE } else { cli.chunk_size as i64 * 1024 * 1024 };
    ...
    println!("  Small files: {} x {} bytes", cli.small_files, SMALL_FILE_SIZE);
    println!("  Medium files: {} x {} MB", cli.medium_files, MEDIUM_FILE_SIZE / (1024 * 1024));
    println!("  Large files: {} x {} GB", cli.large_files, LARGE_FILE_SIZE / (1024 * 1024 * 1024));
    ...
    let (source_root, _tmpdir) = if let Some(ref dir) = cli.source_dir {
        ...
        let tmp = if cli.keep_files {
            ...
            return run_with_tmpdir(cli, worker_counts, is_scaling, chunk_size_bytes, max_memory_bytes, p, Some(t), start_all);
        };
        (tmp, None)
    };

    run_with_tmpdir(cli, worker_counts, is_scaling, chunk_size_bytes, max_memory_bytes, source_root, None::<tempfile::TempDir>, start_all);
}
...
fn run_with_tmpdir(
    ...
    println!("  Total size: {:.2} MB", manifest.total_size as f64 / (1024.0 * 1024.0));
    println!("  Duration: {:.2} seconds", collect_dur);
    println!("  Throughput: {:.0} paths/s", total_paths as f64 / collect_dur);
    ...
    for &workers in &worker_counts {
        let label = if is_scaling { format!(" (w={})", workers) } else { String::new() };
        ...
        {
            println!("\n============================================================");
            println!("TEST: HASH_UPLOAD (filesystem, max_workers={}){}", workers, label);
            println!("============================================================");

            let hash_cache = if cli.no_hash_cache { None } else {
            ...
            println!("  Files processed: {} ({:.2} MB)", stats.uploaded_files, stats.uploaded_bytes as f64 / (1024.0 * 1024.0));
            println!("  Files skipped: {} ({:.2} MB)", stats.skipped_files, stats.skipped_bytes as f64 / (1024.0 * 1024.0));
            println!("  Duration: {:.2} seconds", dur);
            println!("  Throughput: {:.2} MB/s", tp);

            results.push(TimingResult {
                operation: format!("UPLOAD cold{}", label), duration: dur,
                files: stats.total_files, bytes: tb, throughput_mb_s: tp, throughput_paths_s: 0.0,
            });
            ...
            results.push(TimingResult {
                operation: format!("UPLOAD warm{}", label), duration: dur,
                files: stats.total_files, bytes: tb, throughput_mb_s: tp, throughput_paths_s: 0.0,
            });
            ...
            if !cli.skip_download {
                ...
                let rel = subtree_snapshot(hashed_snapshot, source_root.to_str().unwrap(), SymlinkPolicy::CollapseEscaping).unwrap();
                ...
                {
                    println!("\n============================================================");
                    println!("TEST: DOWNLOAD (filesystem, max_workers={}){}", workers, label);
                    println!("============================================================");

                    std::fs::create_dir_all(&download_root).unwrap();
                    std::fs::create_dir_all(&dl_hash_cache_dir).unwrap();

                    let dl_hash_cache = if cli.no_hash_cache { None } else {
                    ...
                    println!("  Total bytes: {:.2} MB", ds.downloaded_bytes as f64 / (1024.0 * 1024.0));
                    println!("  Duration: {:.2} seconds", dur);
                    println!("  Throughput: {:.2} MB/s", tp);

                    results.push(TimingResult {
                        operation: format!("DOWNLOAD cold{}", label), duration: dur,
                        files: ds.downloaded_files, bytes: ds.downloaded_bytes, throughput_mb_s: tp, throughput_paths_s: 0.0,
                    });
                    ...
                    results.push(TimingResult {
                        operation: format!("DOWNLOAD warm{}", label), duration: dur,
                        files: ds2.total_files, bytes: ds.total_bytes, throughput_mb_s: tp, throughput_paths_s: 0.0,
                    });
                }
                        ...
                        for (rel, expected_hash) in expected {
                            ...
                            if !file_path.exists() {
                                ...
                                if actual != *expected_hash {
                                    eprintln!("  MISMATCH: {} (expected {}..., got {}...)", rel, &expected_hash[..16], &actual[..16]);
                                    fail += 1;
                                } else {
                                    ok += 1;
                                }
            ...
            // DIFF
            {
                ...
                println!("============================================================");
                let t = Instant::now();
                let diff = diff_snapshots(hashed_snapshot, hashed_snapshot, &DiffOptions::default()).unwrap();
                let dur = t.elapsed().as_secs_f64();
    ...
    if is_scaling && !all_scaling.is_empty() {
        ...
        let col_names: Vec<&str> = all_scaling[0].1.iter().map(|r| {
        ...
        print!("| {:>8} |", "Workers");
        for c in &col_names { print!(" {:>16} |", c); }
        println!();

        print!("|{:->10}:|", "");
        for _ in &col_names { print!("{:->17}:|", ""); }
        println!();

        for (workers, results) in &all_scaling {
            print!("| {:>8} |", workers);
            for r in results { print!(" {:>16} |", format_duration(r.duration)); }
            println!();
        }
    }
    ...
    println!("SUMMARY");
    println!("============================================================");
    println!("\n  {:30} {:>10} {:>10} {:>12}", "Operation", "Duration", "Files", "Throughput");
    println!("  {:30} {:>10} {:>10} {:>12}", "-".repeat(30), "-".repeat(10), "-".repeat(10), "-".repeat(12));

    println!("  {:30} {:>10} {:>10} {:>12}",
        "COLLECT", format!("{:.2}s", collect_dur), total_paths, format!("{:.0} paths/s", total_paths as f64 / collect_dur));

    if let Some((_, ref results)) = all_scaling.last() {
        for r in results {
            let tp = if r.throughput_mb_s > 0.0 { format!("{:.1} MB/s", r.throughput_mb_s) } else { "N/A".to_string() };
            println!("  {:30} {:>10} {:>10} {:>12}", r.operation, format!("{:.2}s", r.duration), r.files, tp);
        }
    }

    println!("\n  Total test time: {:.2} seconds", start_all.elapsed().as_secs_f64());
    println!("\n✓ ALL TESTS PASSED");
}
