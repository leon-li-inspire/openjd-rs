...
fn test_resolved_symtab_serialize_with_let_bindings() {
    ...
    let params = preprocess_job_parameters(
        &jt, &Default::default(), &[], td.path(), td.path(), true,
    ).unwrap();
    ...
    let rb = json.get("resolvedSymTab").expect("missing resolvedSymTab in JSON");
    let arr = rb.as_array().expect("resolvedSymTab should be array");

    // Find the "x" binding
    let x_binding = arr.iter().find(|v| v["name"] == "x").expect("missing x binding");
    assert_eq!(x_binding["value"], "10");
    assert_eq!(x_binding["type"], "int");

    // Find the "greeting" binding
    let greeting = arr.iter().find(|v| v["name"] == "greeting").expect("missing greeting");
    assert_eq!(greeting["value"], "hello");
    assert_eq!(greeting["type"], "string");

    // Find Job.Name
    let job_name = arr.iter().find(|v| v["name"] == "Job.Name").expect("missing Job.Name");
    assert_eq!(job_name["value"], "TestJob");
    assert_eq!(job_name["type"], "string");

    // Find Step.Name
    let step_name = arr.iter().find(|v| v["name"] == "Step.Name").expect("missing Step.Name");
...
fn test_resolved_symtab_round_trip() {
    ...
    let deserialized: openjd_model::SymbolTable =
        serde_json::from_value(rb_json.clone()).unwrap();

    // The round-tripped symtab should contain the serialized entries
    assert_eq!(deserialized.get_value("count"), Some(&openjd_expr::ExprValue::Int(5)));
    assert_eq!(deserialized.get_value("flag"), Some(&openjd_expr::ExprValue::Bool(true)));
    let original_symtab = original_symtab.to_symtab(openjd_expr::PathFormat::Posix).unwrap();
    assert_eq!(deserialized.get_value("Job.Name"), original_symtab.get_value("Job.Name"));
    assert_eq!(deserialized.get_value("Step.Name"), original_symtab.get_value("Step.Name"));
}
...
fn test_resolved_symtab_list_value() {
    ...
    let my_list = rb.iter().find(|v| v["name"] == "my_list").expect("missing my_list");
    assert_eq!(my_list["type"], "list[int]");
    // Value should be a JSON array of strings (matching Python's transport format)
    let val = my_list["value"].as_array().expect("list value should be array");
    assert_eq!(val, &[serde_json::json!("1"), serde_json::json!("2"), serde_json::json!("3")]);
}
...
fn test_resolved_symtab_serialized_without_expr() {
    ...
    let json = serde_json::to_value(&job.steps[0]).unwrap();
    let rb = json.get("resolvedSymTab").expect("should have resolvedSymTab");
    let arr = rb.as_array().unwrap();
    // Should contain Param.X and RawParam.X
    assert!(arr.iter().any(|v| v["name"] == "Param.X"), "missing Param.X");
    assert!(arr.iter().any(|v| v["name"] == "RawParam.X"), "missing RawParam.X");
}
...
fn test_script_let_apply_path_mapping_not_in_resolved_symtab() {
    ...
    let symtab = job.steps[0].resolved_symtab.as_ref().unwrap().to_symtab(openjd_expr::PathFormat::Posix).unwrap();
    // Script-level let bindings are type-checked but not stored in resolved_symtab
    assert!(symtab.get_value("mapped").is_none(),
...
fn test_script_let_type_error_caught_at_validation() {
    ...
    assert!(err.contains("path") && err.contains("int"),
...
fn test_step_resolved_symtab_excludes_unreferenced_symbols() {
    ...
    let st = job.steps[0].resolved_symtab.as_ref().unwrap()
        .to_symtab(openjd_expr::PathFormat::Posix).unwrap();

    // Referenced symbols are present
    assert!(st.get_value("Param.Used").is_some(), "Param.Used should be in resolved_symtab");
    assert!(st.get_value("referenced").is_some(), "referenced let binding should be in resolved_symtab");

    // Unreferenced symbols are excluded
    assert!(st.get_value("Param.Unused").is_none(), "Param.Unused should be excluded");
    assert!(st.get_value("RawParam.Unused").is_none(), "RawParam.Unused should be excluded");
    assert!(st.get_value("unreferenced").is_none(), "unreferenced let binding should be excluded");
    assert!(st.get_value("Job.Name").is_none(), "Job.Name should be excluded when unreferenced");
    assert!(st.get_value("Step.Name").is_none(), "Step.Name should be excluded when unreferenced");
}
...
fn test_job_env_resolved_symtab_excludes_unreferenced_symbols() {
    ...
    let env_st = env.resolved_symtab.as_ref().expect("job env should have resolved_symtab")
        .to_symtab(openjd_expr::PathFormat::Posix).unwrap();

    assert!(env_st.get_value("Param.EnvUsed").is_some(), "Param.EnvUsed should be in env resolved_symtab");
    assert!(env_st.get_value("Param.StepOnly").is_none(), "Param.StepOnly should be excluded from env resolved_symtab");
    assert!(env_st.get_value("Job.Name").is_none(), "Job.Name should be excluded when unreferenced by env");

    // Check the step's resolved_symtab has the opposite
    let step_st = job.steps[0].resolved_symtab.as_ref().unwrap()
        .to_symtab(openjd_expr::PathFormat::Posix).unwrap();

    assert!(step_st.get_value("Param.StepOnly").is_some(), "Param.StepOnly should be in step resolved_symtab");
    assert!(step_st.get_value("Param.EnvUsed").is_none(), "Param.EnvUsed should be excluded from step resolved_symtab");
}
