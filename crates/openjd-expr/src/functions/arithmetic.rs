...
pub fn add_int(_: Ctx, a: &[ExprValue]) -> R {
    match (&a[0], &a[1]) {
        (ExprValue::Int(l), ExprValue::Int(r)) => Ok(ExprValue::Int(l.checked_add(*r).ok_or_else(|| ExpressionError::integer_overflow())?)),
...
pub fn sub_int(_: Ctx, a: &[ExprValue]) -> R {
    match (&a[0], &a[1]) {
        (ExprValue::Int(l), ExprValue::Int(r)) => Ok(ExprValue::Int(l.checked_sub(*r).ok_or_else(|| ExpressionError::integer_overflow())?)),
        _ => Err(ExpressionError::type_error("type error"))
    }
}
...
pub fn mul_int(_: Ctx, a: &[ExprValue]) -> R {
    match (&a[0], &a[1]) {
        (ExprValue::Int(l), ExprValue::Int(r)) => Ok(ExprValue::Int(l.checked_mul(*r).ok_or_else(|| ExpressionError::integer_overflow())?)),
        _ => Err(ExpressionError::type_error("type error"))
    }
}
...
pub fn floordiv_int(_: Ctx, a: &[ExprValue]) -> R {
    match (&a[0], &a[1]) {
        (ExprValue::Int(l), ExprValue::Int(r)) => {
            if *r == 0 { return Err(ExpressionError::division_by_zero("Division")); }
            let d = l.checked_div(*r).ok_or_else(|| ExpressionError::integer_overflow())?;
...
pub fn mod_int(_: Ctx, a: &[ExprValue]) -> R {
    match (&a[0], &a[1]) {
        (ExprValue::Int(l), ExprValue::Int(r)) => {
            if *r == 0 { return Err(ExpressionError::division_by_zero("Modulo")); }
            let rem = l.checked_rem(*r).ok_or_else(|| ExpressionError::integer_overflow())?;
            // Python uses floored modulo: result sign matches divisor sign.
            let result = if rem != 0 && (rem ^ r) < 0 { rem + r } else { rem };
            Ok(ExprValue::Int(result))
        }
        _ => Err(ExpressionError::type_error("type error"))
    }
...
pub fn pow_int(_: Ctx, a: &[ExprValue]) -> R {
    match (&a[0], &a[1]) {
        (ExprValue::Int(base), ExprValue::Int(exp)) => {
            if *exp < 0 {
                if *base == 0 { return Err(ExpressionError::float_error("Cannot raise zero to a negative power")); }
            ...
            if *exp > 63 && !matches!(*base, 0 | 1 | -1) {
                return Err(ExpressionError::integer_overflow());
            }
            Ok(ExprValue::Int(base.checked_pow(*exp as u32).ok_or_else(|| ExpressionError::integer_overflow())?))
        }
        _ => Err(ExpressionError::type_error("type error"))
    }
}
...
pub fn neg_int(_: Ctx, a: &[ExprValue]) -> R {
    match &a[0] { ExprValue::Int(n) => Ok(ExprValue::Int(n.checked_neg().ok_or_else(ExpressionError::integer_overflow)?)), _ => Err(ExpressionError::type_error("type error")) }
}

pub fn pos_int(_: Ctx, a: &[ExprValue]) -> R {
    match &a[0] { ExprValue::Int(n) => Ok(ExprValue::Int(*n)), _ => Err(ExpressionError::type_error("type error")) }
}
...
pub fn truediv_float(_: Ctx, a: &[ExprValue]) -> R {
    let (l, r) = get_two_floats(a)?;
    if r == 0.0 { return Err(ExpressionError::division_by_zero("Division")); }
    Ok(ExprValue::Float(Float64::new(l / r)?))
}
...
pub fn mod_float(_: Ctx, a: &[ExprValue]) -> R {
    let (l, r) = get_two_floats(a)?;
    if r == 0.0 { return Err(ExpressionError::division_by_zero("Modulo")); }
...
pub fn pow_float(_: Ctx, a: &[ExprValue]) -> R {
    let (l, r) = get_two_floats(a)?;
    if l == 0.0 && r < 0.0 { return Err(ExpressionError::float_error("Cannot raise zero to a negative power")); }
    if l < 0.0 && r.fract() != 0.0 {
        return Err(ExpressionError::float_error(format!("Cannot compute {} ** {} (would produce complex number)", l, r)));
    }
    let result = l.powf(r);
    if result.is_infinite() {
        return Err(ExpressionError::float_error(format!("Overflow computing {} ** {} (result too large for float)", l, r)));
    }
    Ok(ExprValue::Float(Float64::new(result)?))
}

pub fn neg_float(_: Ctx, a: &[ExprValue]) -> R {
    match &a[0] { ExprValue::Float(n) => Ok(ExprValue::Float(Float64::new(-n.0)?)), _ => Err(ExpressionError::type_error("type error")) }
}
...
pub fn mul_string(ctx: Ctx, a: &[ExprValue]) -> R {
    match (&a[0], &a[1]) {
        (ExprValue::String(s), ExprValue::Int(n)) => {
            if *n < 0 { return Ok(ExprValue::String(String::new())); }
...
pub fn path_div(ctx: Ctx, a: &[ExprValue]) -> R {
    ...
    ctx.count_string_ops(l.len() + r.len())?;
    if r.starts_with('/') || r.starts_with("\\\\") || (r.len() >= 3 && r.as_bytes()[0].is_ascii_alphabetic() && r.as_bytes()[1] == b':' && (r.as_bytes()[2] == b'/' || r.as_bytes()[2] == b'\\')) {
        return Ok(ExprValue::Path { value: r.to_string(), format });
    }
    let sep = if crate::uri_path::is_uri(l) { "/" } else if format == PathFormat::Windows { "\\" } else { "/" };
    let left = l.trim_end_matches(|c: char| c == '/' || c == '\\');
    Ok(ExprValue::Path { value: format!("{left}{sep}{r}"), format })
}
...
pub fn add_path_string(ctx: Ctx, a: &[ExprValue]) -> R {
    match (&a[0], &a[1]) {
        (ExprValue::Path { value: l, format }, ExprValue::String(r)) => {
            ctx.count_string_ops(l.len() + r.len())?;
            Ok(ExprValue::Path { value: format!("{l}{r}"), format: *format })
        }
        _ => Err(ExpressionError::type_error("type error"))
    }
}
...
pub fn add_list_range(ctx: Ctx, a: &[ExprValue]) -> R {
    let (mut l, et) = a[0].clone().into_list().ok_or_else(|| ExpressionError::type_error("type error"))?;
...
pub fn add_range_list(ctx: Ctx, a: &[ExprValue]) -> R {
    let r = match &a[0] { ExprValue::RangeExpr(r) => r, _ => return Err(ExpressionError::type_error("type error")) };
    let (l, et) = a[1].clone().into_list().ok_or_else(|| ExpressionError::type_error("type error"))?;
...
fn get_two_floats(a: &[ExprValue]) -> Result<(f64, f64), ExpressionError> {
    let l = match &a[0] { ExprValue::Float(f) => f.0, ExprValue::Int(i) => *i as f64, _ => return Err(ExpressionError::type_error("type error")) };
    let r = match &a[1] { ExprValue::Float(f) => f.0, ExprValue::Int(i) => *i as f64, _ => return Err(ExpressionError::type_error("type error")) };
    Ok((l, r))
}
