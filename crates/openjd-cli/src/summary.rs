...
pub fn execute(args: SummaryArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let param_order: Vec<String> = job_template.parameter_definitions_list().iter()
        .map(|p| p.name().to_string())
        .collect();
    let param_descriptions: std::collections::HashMap<&str, &str> = job_template.parameter_definitions_list().iter()
    ...
    for env_path in &args.environments {
        ...
        env_templates.push(parse::decode_environment_template(env_value, Some(&supported_exts))?);
    }

    // Parse parameters
    let input_values = crate::run::parse_cli_parameters(&args.parameters)?;
    let job_template_dir = crate::run::strip_extended_prefix(&std::fs::canonicalize(path)?
    ...
    let param_values = openjd_model::preprocess_job_parameters(
        &job_template, &input_values, &env_templates,
        &job_template_dir, &current_working_dir, false,
    )?;
...
fn output_job_summary(job: &job::Job, param_order: &[String], param_descriptions: &std::collections::HashMap<&str, &str>, output_format: &str) -> Result<(), Box<dyn std::error::Error>> {
    // Collect step summaries
    let mut step_envs_info: Vec<EnvInfo> = Vec::new();
    let step_summaries: Vec<StepInfo> = job.steps.iter().map(|s| {
        let total_tasks = step_total_tasks(s);
        let task_params: Vec<(String, String)> = s.parameter_space.as_ref()
            .map(|ps| {
                let mut v: Vec<_> = ps.task_parameter_definitions.iter()
            ...
            .unwrap_or_default();
        let envs: Vec<String> = s.step_environments.as_ref()
            .map(|envs| envs.iter().map(|e| e.name.clone()).collect())
            .unwrap_or_default();
        if let Some(envs) = &s.step_environments {
            for e in envs {
                step_envs_info.push(EnvInfo { name: e.name.clone(), description: e.description.clone(), parent: s.name.clone() });
            }
        }
        let deps: Vec<String> = s.dependencies.as_ref()
            .map(|deps| deps.iter().map(|d| d.depends_on.clone()).collect())
            .unwrap_or_default();
        StepInfo { name: s.name.clone(), description: s.description.clone(), total_tasks, task_params, envs, deps }
    }).collect();
    ...
    let root_envs: Vec<EnvInfo> = job.job_environments.as_ref()
        .map(|envs| envs.iter().map(|e| EnvInfo { name: e.name.clone(), description: e.description.clone(), parent: "root".into() }).collect())
        .unwrap_or_default();
    let total_envs = root_envs.len() + step_envs;

    // Collect parameter summaries in template definition order
    let params: Vec<ParamInfo> = param_order.iter()
        .filter_map(|name| job.parameters.get(name).map(|p| ParamInfo {
            name: name.clone(),
...
fn output_step_summary(job: &job::Job, step_name: &str, output_format: &str) -> Result<(), Box<dyn std::error::Error>> {
    let step = job.steps.iter().find(|s| s.name == step_name)
        .ok_or_else(|| format!("Step '{step_name}' does not exist in Job '{}'.", job.name))?;

    let total_tasks = step_total_tasks(step);
    let task_params: Vec<(String, String)> = step.parameter_space.as_ref()
        .map(|ps| {
            let mut v: Vec<_> = ps.task_parameter_definitions.iter()
                .map(|(name, tp)| (name.clone(), task_param_type_name(tp).to_string()))
                .collect();
        ...
        .unwrap_or_default();
    let envs: Vec<EnvInfo> = step.step_environments.as_ref()
        .map(|envs| envs.iter().map(|e| EnvInfo { name: e.name.clone(), description: e.description.clone(), parent: step.name.clone() }).collect())
        .unwrap_or_default();
...
impl CliResult for JobSummaryResult {
    fn to_json_value(&self) -> serde_json::Value {
        ...
        obj.insert("message".into(), format!("Summary for '{}'", self.name).into());
        obj.insert("name".into(), self.name.clone().into());
        if !self.params.is_empty() {
            obj.insert("parameter_definitions".into(), serde_json::json!(
                self.params.iter().map(|p| {
                    let mut m = serde_json::Map::new();
                    m.insert("name".into(), p.name.clone().into());
                    if let Some(d) = &p.description { m.insert("description".into(), d.clone().into()); }
        ...
        obj.insert("total_tasks".into(), serde_json::Value::Number(self.total_tasks.into()));
        if self.total_envs > 0 {
            obj.insert("total_environments".into(), self.total_envs.into());
        }
        if !self.root_envs.is_empty() {
            obj.insert("root_environments".into(), serde_json::json!(
                self.root_envs.iter().map(|e| serde_json::json!({"name": e.name, "parent": e.parent})).collect::<Vec<_>>()
            ));
        }
        let steps_json: Vec<serde_json::Value> = self.step_summaries.iter().map(|s| {
            let mut m = serde_json::Map::new();
            m.insert("name".into(), s.name.clone().into());
            if let Some(d) = &s.description { m.insert("description".into(), d.clone().into()); }
            m.insert("total_tasks".into(), s.total_tasks.into());
            if !s.task_params.is_empty() {
                m.insert("parameter_definitions".into(), serde_json::json!(
                    s.task_params.iter().map(|(n, t)| serde_json::json!({"name": n, "type": t})).collect::<Vec<_>>()
                ));
            }
            if !s.envs.is_empty() { m.insert("environments".into(), s.envs.len().into()); }
            if !s.deps.is_empty() { m.insert("dependencies".into(), s.deps.len().into()); }
            serde_json::Value::Object(m)
        }).collect();
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        for (i, s) in self.step_summaries.iter().enumerate() {
            ...
            if !s.task_params.is_empty() {
                ...
                for (name, tp) in &s.task_params { writeln!(f, "    - {name} ({tp})")?; }
            }
            if !s.envs.is_empty() { writeln!(f, "  {} environments", s.envs.len())?; }
            if !s.deps.is_empty() { writeln!(f, "  {} dependencies", s.deps.len())?; }
            writeln!(f)?;
        }
        if self.total_envs > 0 {
            writeln!(f)?;
            ...
            for e in &self.root_envs {
                ...
                if let Some(d) = &e.description { write!(f, "\n {d}")?; }
                writeln!(f)?;
            }
...
impl CliResult for StepSummaryResult {
    fn to_json_value(&self) -> serde_json::Value {
        let mut obj = serde_json::Map::new();
        obj.insert("status".into(), "success".into());
        obj.insert("message".into(), format!("Summary for Step '{}' in Job '{}'", self.step_name, self.job_name).into());
        ...
        if !self.deps.is_empty() {
            obj.insert("dependencies".into(), serde_json::json!(self.deps.iter().map(|d| serde_json::json!({"step_name": d})).collect::<Vec<_>>()));
        }
        if !self.task_params.is_empty() {
            obj.insert("parameter_definitions".into(), serde_json::json!(self.task_params.iter().map(|(n, t)| serde_json::json!({"name": n, "type": t})).collect::<Vec<_>>()));
        }
        serde_json::Value::Object(obj)
    }
}
...
impl fmt::Display for StepSummaryResult {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f)?;
        writeln!(f, "--- Summary for Step '{}' in Job '{}' ---", self.step_name, self.job_name)?;
        writeln!(f)?;
        writeln!(f, "Total tasks: {}", self.total_tasks)?;
        ...
        if !self.deps.is_empty() {
            ...
            for d in &self.deps { writeln!(f, "- '{d}'")?; }
        }
        if !self.task_params.is_empty() {
            writeln!(f)?;
            writeln!(f, "Parameters:")?;
            for (name, tp) in &self.task_params { writeln!(f, "- {name} ({tp})")?; }
        }
