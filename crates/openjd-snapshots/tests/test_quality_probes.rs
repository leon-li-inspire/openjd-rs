...
fn validate_allows_duplicate_file_paths() {
    ...
    assert!(result.is_ok(), "BUG DEMONSTRATION: validate() accepts duplicate paths — this test documents the gap");
}
...
fn validate_allows_duplicate_dir_paths() {
    let m: Snapshot = Manifest::new(HashAlgorithm::Xxh128, WHOLE_FILE_CHUNK_SIZE)
        .with_dirs(vec![
            DirEntry { path: "dir".into(), deleted: false },
            DirEntry { path: "dir".into(), deleted: false },
        ]);
    let result = m.validate();
    assert!(result.is_ok(), "BUG DEMONSTRATION: validate() accepts duplicate dir paths");
}
...
fn hash_file_chunked_consistent_chunk_count() {
    ...
    let data2: Vec<u8> = (0..(3 * chunk_size + 500)).map(|i| (i % 256) as u8).collect();
...
fn manifest_deserialization_ignores_phantom_types() {
    let abs: AbsSnapshot = Manifest::new(HashAlgorithm::Xxh128, WHOLE_FILE_CHUNK_SIZE)
    ...
    assert!(rel.is_ok(), "BUG DEMONSTRATION: deserialization doesn't enforce path type constraints");

    // But validate() catches it
    let rel = rel.unwrap();
    assert!(rel.validate().is_err(), "validate() correctly rejects absolute paths in Rel manifest");
}
