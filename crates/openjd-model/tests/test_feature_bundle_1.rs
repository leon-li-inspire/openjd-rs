...
fn decode_fb1(s: &str) {
    let v = yaml_val(s);
    decode_job_template(v, Some(&["FEATURE_BUNDLE_1"])).expect(&format!("Expected success for: {s}"));
}

/// Decode without any extensions supported.
fn check_err_no_ext(s: &str, expected: &[&str]) {
    let v = yaml_val(s);
    let err = decode_job_template(v, Some(&[]))
...
fn param_count_201_with_extension_fails() {
    let params: Vec<String> = (0..201).map(|i| format!(r#"{{"name": "P{i}", "type": "INT"}}"#)).collect();
    let s = format!(r#"{{
        "specificationVersion": "jobtemplate-2023-09",
        "extensions": ["FEATURE_BUNDLE_1"],
        "name": "Test",
...
fn param_count_200_with_extension_succeeds() {
    let params: Vec<String> = (0..200).map(|i| format!(r#"{{"name": "P{i}", "type": "INT"}}"#)).collect();
    let s = format!(r#"{{
        "specificationVersion": "jobtemplate-2023-09",
        "extensions": ["FEATURE_BUNDLE_1"],
        "name": "Test",
