...
pub fn parse_cli_parameters(args: &[String]) -> Result<HashMap<String, openjd_expr::ExprValue>, Box<dyn std::error::Error>> {
    ...
    for arg in args {
        ...
        if arg.starts_with("file://") {
            ...
            if !path.exists() {
                return Err(format!("Provided parameter file '{}' does not exist.", path.display()).into());
            }
            ...
            if let Some(obj) = value.as_object() {
                ...
                return Err(format!("Job parameter file '{}' should contain a dictionary.", arg).into());
            }
            ...
            params.insert(key.to_string(), openjd_expr::ExprValue::String(value.to_string()));
        } else {
...
pub fn parse_task_params(args: &[String]) -> Result<HashMap<String, String>, Box<dyn std::error::Error>> {
    let mut params = HashMap::new();
        ...
        if let Some((name, value)) = arg.split_once('=') {
            if !name.is_empty() && !value.is_empty() {
                if params.contains_key(name) {
                    errors.push(format!("Task parameter '{name}' has been defined more than once."));
                } else {
                    params.insert(name.to_string(), value.to_string());
                }
            } else {
                errors.push(format!("Task parameter '{arg}' defined incorrectly. Expected '<NAME>=<VALUE>' format."));
            }
        } else {
            errors.push(format!("Task parameter '{arg}' defined incorrectly. Expected '<NAME>=<VALUE>' format."));
        }
    ...
    if !errors.is_empty() {
        let msg = errors.iter().map(|e| format!("\n - {e}")).collect::<String>();
...
pub fn parse_tasks_arg(arg: &str) -> Result<Vec<HashMap<String, String>>, Box<dyn std::error::Error>> {
    let arg = arg.trim();
    ...
    for item in arr {
        ...
        for (k, v) in obj {
            let s = match v {
                ...
                _ => return Err("--tasks argument must be a list of maps from string to string when decoded.".into()),
            };
        ...
        let (param_type, expr_value) = match tp {
            TaskParameter::Int { .. } => {
                let v: i64 = value.parse().map_err(|_|
                    format!("Task parameter '{name}': expected INT, got '{value}'"))?;
                (TaskParameterType::Int, openjd_expr::ExprValue::Int(v))
            }
            TaskParameter::Float { .. } => {
                let v: f64 = value.parse().map_err(|_|
                    format!("Task parameter '{name}': expected FLOAT, got '{value}'"))?;
                (TaskParameterType::Float, openjd_expr::ExprValue::Float(openjd_expr::value::Float64::new(v).unwrap()))
            }
            TaskParameter::String { .. } => {
                (TaskParameterType::String, openjd_expr::ExprValue::String(value.clone()))
            }
            TaskParameter::Path { .. } => {
                (TaskParameterType::Path, openjd_expr::ExprValue::Path {
                    value: value.clone(),
                    format: openjd_expr::PathFormat::host(),
                })
            }
            TaskParameter::ChunkInt { .. } => {
                let r: openjd_expr::RangeExpr = value.parse().map_err(|e: openjd_expr::ExpressionError|
                    format!("Task parameter '{name}': invalid range expression '{value}': {e}"))?;
                (TaskParameterType::ChunkInt, openjd_expr::ExprValue::RangeExpr(r))
            }
        };
        result.insert(name.clone(), TaskParameterValue { param_type, value: expr_value });
    }
    Ok(result)
}

/// Load path mapping rules from a file or inline JSON string.
pub fn load_path_mapping_rules(path: &Option<String>) -> Result<Vec<PathMappingRule>, Box<dyn std::error::Error>> {
    let Some(path_str) = path else { return Ok(Vec::new()) };
    let path_str = path_str.trim();

    let content = if path_str.starts_with("file://") {
        let file_path = std::path::Path::new(path_str.trim_start_matches("file://"));
        std::fs::read_to_string(file_path)
            .map_err(|e| format!("Cannot read path mapping rules file '{}': {e}", file_path.display()))?
    } else {
        path_str.to_string()
    };
    ...
    if !doc.is_object() {
        return Err("Path mapping rules must be an object with 'version' and 'path_mapping_rules' fields".into());
    }
    match doc.get("version").and_then(|v| v.as_str()) {
        Some("pathmapping-1.0") => {}
        _ => return Err("Path mapping rules must have a 'version' value of 'pathmapping-1.0'".into()),
    }
    let rules = doc.get("path_mapping_rules")
        .and_then(|v| v.as_array())
        .ok_or("Path mapping rules must contain a list named 'path_mapping_rules'")?;
    rules.iter().map(|r| {
        serde_json::from_value(r.clone()).map_err(|e| e.into())
    }).collect()
}
