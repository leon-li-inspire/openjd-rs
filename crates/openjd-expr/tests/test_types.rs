...
#[test] fn equality() {
    ...
    assert_ne!(ExprType::list(ExprType::INT), ExprType::list(ExprType::STRING));
}
...
#[test] fn parse_signature_union_return() {
...
#[test] fn parse_signature_no_params() {
    let t = p("() -> string");
    assert_eq!(t.code(), TypeCode::Signature);
...
#[test] fn parse_signature_generic() {
...
#[test] fn parse_nested_list() {
    assert_eq!(p("list[list[int]]"), ExprType::list(ExprType::list(ExprType::INT)));
}

#[test] fn parse_optional_types() {
    let int_opt = p("int?");
...
#[test] fn noreturn_collapses_in_union() {
    assert_eq!(ExprType::union(vec![ExprType::INT, ExprType::NORETURN]), ExprType::INT);
}

#[test] fn noreturn_collapses_with_multiple() {
...
#[test] fn noreturn_only_union() {
    assert_eq!(ExprType::union(vec![ExprType::NORETURN, ExprType::NORETURN]), ExprType::NORETURN);
}

#[test] fn union_construction() {
    let u = ExprType::union(vec![ExprType::INT, ExprType::STRING]);
    assert_eq!(u.code(), TypeCode::Union);
    assert_eq!(u.to_string(), "int | string");
}

#[test] fn union_deduplication() {
    let u = ExprType::union(vec![ExprType::INT, ExprType::INT]);
    assert_eq!(u.code(), TypeCode::Int);
}

#[test] fn union_single_element_unwrap() {
    let u = ExprType::union(vec![ExprType::STRING]);
    assert_eq!(u.code(), TypeCode::String);
}

#[test] fn union_flattening() {
...
#[test] fn union_any_absorption() {
    assert_eq!(ExprType::union(vec![ExprType::INT, ExprType::ANY]).code(), TypeCode::Any);
}

#[test] fn union_string_parsing() {
    let u = p("int | string");
    assert_eq!(u.code(), TypeCode::Union);
    assert_eq!(u.to_string(), "int | string");
}

#[test] fn union_string_parsing_multiple() {
...
#[test] fn union_equality_order_independent() {
    let u1 = ExprType::union(vec![ExprType::INT, ExprType::STRING]);
...
#[test] fn union_matches_union_with_overlap() {
    let u1 = ExprType::union(vec![ExprType::INT, ExprType::STRING]);
...
#[test] fn typevar_binds_to_union() {
    let u = ExprType::union(vec![ExprType::INT, ExprType::STRING]);
...
#[test] fn nested_typevar_binds_to_union() {
...
#[test] fn unknown_construct_with_constraint() {
    let t = ExprType::unresolved(ExprType::INT);
    assert_eq!(t.code(), TypeCode::Unresolved);
    assert_eq!(t.params(), vec![ExprType::INT]);
}

#[test] fn unknown_parse_bare() {
    let t = p("unresolved");
    assert_eq!(t.code(), TypeCode::Unresolved);
    assert_eq!(t.params(), vec![ExprType::ANY]);
}

#[test] fn unknown_parse_int() {
    let t = p("unresolved[int]");
    assert_eq!(t.code(), TypeCode::Unresolved);
    assert_eq!(t.params(), vec![ExprType::INT]);
}

#[test] fn unknown_parse_list() {
    let t = p("unresolved[list[string]]");
    assert_eq!(t.code(), TypeCode::Unresolved);
    assert_eq!(t.params(), vec![ExprType::list(ExprType::STRING)]);
}

#[test] fn unknown_parse_union() {
    let t = p("unresolved[int | float]");
    assert_eq!(t.code(), TypeCode::Unresolved);
    assert_eq!(t.params()[0].code(), TypeCode::Union);
}

#[test] fn unknown_parse_any() {
    assert_eq!(p("unresolved[any]"), p("unresolved"));
}

#[test] fn unknown_str_bare() { assert_eq!(p("unresolved").to_string(), "unresolved"); }
#[test] fn unknown_str_any_is_bare() { assert_eq!(ExprType::unresolved(ExprType::ANY).to_string(), "unresolved"); }
#[test] fn unknown_str_int() { assert_eq!(p("unresolved[int]").to_string(), "unresolved[int]"); }
#[test] fn unknown_str_list() { assert_eq!(p("unresolved[list[string]]").to_string(), "unresolved[list[string]]"); }

#[test] fn unknown_roundtrip_bare() {
    let t = p("unresolved");
    assert_eq!(p(&t.to_string()), t);
}

#[test] fn unknown_roundtrip_constrained() {
...
#[test] fn unknown_match_any_constraint() {
    let t = p("unresolved");
    assert!(t.match_type(&ExprType::INT).is_some());
...
#[test] fn match_two_independent_typevars() {
...
#[test] fn match_t_and_t1_are_distinct() {
...
#[test] fn match_contains_signature() {
...
#[test] fn match_conflicting_bindings_fail() {
...
#[test] fn match_nested_list_consistent_binding() {
...
#[test] fn match_nested_list_conflicting_inner() {
...
#[test] fn match_getitem_list_int() {
    let list_t1 = ExprType::list(ExprType::T1);
...
#[test] fn match_getitem_list_string() {
    let list_t1 = ExprType::list(ExprType::T1);
    let b = list_t1.match_type(&ExprType::list(ExprType::STRING)).unwrap();
    let ret = ExprType::T1.substitute(&b);
    assert_eq!(ret, ExprType::STRING);
}

#[test] fn match_getitem_list_list_int() {
    // list[list[int]][0] -> list[int]
    let list_t1 = ExprType::list(ExprType::T1);
    let b = list_t1.match_type(&ExprType::list(ExprType::list(ExprType::INT))).unwrap();
    let ret = ExprType::T1.substitute(&b);
...
#[test] fn match_list_concat_same_type() {
...
#[test] fn match_list_concat_different_types() {
    let sig1 = ExprType::list(ExprType::T1);
    let sig2 = ExprType::list(ExprType::T2);
    let b1 = sig1.match_type(&ExprType::list(ExprType::INT)).unwrap();
...
#[test] fn substitute_multiple_typevars() {
    ...
    assert_eq!(ret_type.substitute(&bindings), ExprType::list(ExprType::FLOAT));
}

#[test] fn substitute_nested() {
    ...
    assert_eq!(t.substitute(&b), ExprType::list(ExprType::list(ExprType::INT)));
}

#[test] fn substitute_no_match_passthrough() {
...
#[test] fn substitute_in_union() {
    // T1 | string with T1=int -> int | string
    let t = ExprType::union(vec![ExprType::T1, ExprType::STRING]);
    let mut b = HashMap::new();
    b.insert(TypeCode::TypeVarT1, ExprType::INT);
    let result = t.substitute(&b);
    assert_eq!(result, ExprType::union(vec![ExprType::INT, ExprType::STRING]));
}

#[test] fn substitute_in_unresolved() {
...
#[test] fn match_concrete_same() {
    assert_eq!(ExprType::INT.match_type(&ExprType::INT), Some(HashMap::new()));
}

#[test] fn match_concrete_different() {
    assert!(ExprType::INT.match_type(&ExprType::STRING).is_none());
}

#[test] fn match_concrete_list_same() {
    assert_eq!(
...
#[test] fn match_concrete_list_different_elem() {
    assert!(ExprType::list(ExprType::INT).match_type(&ExprType::list(ExprType::STRING)).is_none());
}

#[test] fn match_list_vs_nonlist() {
    assert!(ExprType::list(ExprType::INT).match_type(&ExprType::INT).is_none());
    assert!(ExprType::INT.match_type(&ExprType::list(ExprType::INT)).is_none());
}

// -- Typevar in union matching --

#[test] fn match_typevar_in_union_signature() {
...
#[test] fn match_typevar_in_union_matches_member() {
...
#[test] fn all_four_typevars_distinct() {
...
#[test] fn match_all_four_independently() {
...
#[test] fn sig_display_no_params() {
    let sig = ExprType::signature(vec![], ExprType::INT);
    assert_eq!(sig.to_string(), "() -> int");
}

#[test] fn sig_display_one_param() {
    let sig = ExprType::signature(vec![ExprType::STRING], ExprType::INT);
    assert_eq!(sig.to_string(), "(string) -> int");
}

#[test] fn sig_display_two_params() {
    let sig = ExprType::signature(vec![ExprType::STRING, ExprType::STRING], ExprType::BOOL);
    assert_eq!(sig.to_string(), "(string, string) -> bool");
}

#[test] fn sig_display_generic() {
    let sig = ExprType::signature(vec![ExprType::list(ExprType::T1), ExprType::INT], ExprType::T1);
    assert_eq!(sig.to_string(), "(list[T1], int) -> T1");
}

#[test] fn sig_display_contains() {
    let sig = ExprType::signature(vec![ExprType::list(ExprType::T1), ExprType::T1], ExprType::BOOL);
    assert_eq!(sig.to_string(), "(list[T1], T1) -> bool");
}

// -- Parsing --

#[test] fn sig_parse_no_params() {
    assert_eq!(p("() -> int"), ExprType::signature(vec![], ExprType::INT));
}

#[test] fn sig_parse_one_param() {
    assert_eq!(p("(string) -> int"), ExprType::signature(vec![ExprType::STRING], ExprType::INT));
}

#[test] fn sig_parse_two_params() {
    assert_eq!(
...
#[test] fn sig_parse_generic() {
    assert_eq!(
        p("(list[T1], int) -> T1"),
        ExprType::signature(vec![ExprType::list(ExprType::T1), ExprType::INT], ExprType::T1)
    );
}

#[test] fn sig_parse_contains() {
    assert_eq!(
        p("(list[T1], T1) -> bool"),
        ExprType::signature(vec![ExprType::list(ExprType::T1), ExprType::T1], ExprType::BOOL)
    );
}

#[test] fn sig_parse_complex_return() {
    assert_eq!(
...
#[test] fn match_call_concrete_exact() {
    // (string, string) -> bool matched with (string, string)
    let sig = p("(string, string) -> bool");
    let b = sig.match_call(&[ExprType::STRING, ExprType::STRING]).unwrap();
    assert!(b.is_empty());
}

#[test] fn match_call_concrete_wrong_type() {
    let sig = p("(string, string) -> bool");
    assert!(sig.match_call(&[ExprType::STRING, ExprType::INT]).is_none());
}

#[test] fn match_call_wrong_arity() {
    let sig = p("(string) -> int");
    assert!(sig.match_call(&[ExprType::STRING, ExprType::STRING]).is_none());
    assert!(sig.match_call(&[]).is_none());
}

#[test] fn match_call_getitem() {
    // __getitem__(list[T1], int) -> T1
    let sig = p("(list[T1], int) -> T1");
    let b = sig.match_call(&[ExprType::list(ExprType::INT), ExprType::INT]).unwrap();
    assert_eq!(b[&TypeCode::TypeVarT1], ExprType::INT);
}

#[test] fn match_call_contains_consistent() {
    // __contains__(list[T1], T1) -> bool
...
#[test] fn match_call_contains_conflict() {
    // __contains__(list[T1], T1) -> bool with list[int] and string -> conflict
    let sig = p("(list[T1], T1) -> bool");
    assert!(sig.match_call(&[ExprType::list(ExprType::INT), ExprType::STRING]).is_none());
}

#[test] fn match_call_eq_any_types() {
...
#[test] fn resolve_getitem_int() {
    let sig = p("(list[T1], int) -> T1");
    let ret = sig.resolve_call(&[ExprType::list(ExprType::INT), ExprType::INT]).unwrap();
    assert_eq!(ret, ExprType::INT);
}

#[test] fn resolve_getitem_string() {
    let sig = p("(list[T1], int) -> T1");
    let ret = sig.resolve_call(&[ExprType::list(ExprType::STRING), ExprType::INT]).unwrap();
    assert_eq!(ret, ExprType::STRING);
}

#[test] fn resolve_getitem_nested() {
    // list[list[int]][0] -> list[int]
    let sig = p("(list[T1], int) -> T1");
    let ret = sig.resolve_call(&[ExprType::list(ExprType::list(ExprType::INT)), ExprType::INT]).unwrap();
    assert_eq!(ret, ExprType::list(ExprType::INT));
}

#[test] fn resolve_contains() {
    let sig = p("(list[T1], T1) -> bool");
    let ret = sig.resolve_call(&[ExprType::list(ExprType::INT), ExprType::INT]).unwrap();
    assert_eq!(ret, ExprType::BOOL);
}

#[test] fn resolve_no_match_returns_none() {
    let sig = p("(list[T1], T1) -> bool");
    assert!(sig.resolve_call(&[ExprType::list(ExprType::INT), ExprType::STRING]).is_none());
}

#[test] fn resolve_concrete_sig() {
    let sig = p("(string) -> int");
    assert_eq!(sig.resolve_call(&[ExprType::STRING]).unwrap(), ExprType::INT);
}

#[test] fn resolve_sorted() {
    // sorted(list[T1]) -> list[T1]
    let sig = p("(list[T1]) -> list[T1]");
    let ret = sig.resolve_call(&[ExprType::list(ExprType::STRING)]).unwrap();
    assert_eq!(ret, ExprType::list(ExprType::STRING));
}

#[test] fn resolve_list_mul() {
    // list * int: (list[T1], int) -> list[T1]
    let sig = p("(list[T1], int) -> list[T1]");
    let ret = sig.resolve_call(&[ExprType::list(ExprType::FLOAT), ExprType::INT]).unwrap();
    assert_eq!(ret, ExprType::list(ExprType::FLOAT));
}

// -- is_symbolic / is_concrete for signatures --

#[test] fn sig_is_symbolic() {
    assert!(p("(list[T1], int) -> T1").is_symbolic());
    assert!(!p("(string, string) -> bool").is_symbolic());
}

#[test] fn sig_not_concrete() {
...
#[test] fn value_direct_construction() {
    assert_eq!(ExprValue::from(42i64).expr_type(), ExprType::INT);
    assert_eq!(ExprValue::Float(Float64::new(3.14).unwrap()).expr_type(), ExprType::FLOAT);
...
#[test] fn value_list_path_string_mix() {
    let items = vec![
...
#[test] fn value_type_coercion_str_to_int() {
    let val = ExprValue::from_str_coerce("42", &ExprType::INT, PathFormat::Posix).unwrap();
    assert_eq!(val, ExprValue::Int(42));
}

#[test] fn value_type_coercion_str_to_float() {
    let val = ExprValue::from_str_coerce("3.14", &ExprType::FLOAT, PathFormat::Posix).unwrap();
    assert_eq!(val.expr_type(), ExprType::FLOAT);
}

#[test] fn value_type_coercion_str_to_bool() {
    let val_true = ExprValue::from_str_coerce("true", &ExprType::BOOL, PathFormat::Posix).unwrap();
    assert_eq!(val_true, ExprValue::Bool(true));
    let val_false = ExprValue::from_str_coerce("false", &ExprType::BOOL, PathFormat::Posix).unwrap();
    assert_eq!(val_false, ExprValue::Bool(false));
}

#[test] fn value_type_coercion_str_to_path() {
...
#[test] fn value_unresolved_creation() {
...
#[test] fn value_unresolved_from_list_string_type() {
    let v = ExprValue::unresolved(ExprType::list(ExprType::STRING));
    assert_eq!(v.expr_type(), p("unresolved[list[string]]"));
}

#[test] fn value_unresolved_equality() {
    assert_eq!(ExprValue::unresolved(ExprType::INT), ExprValue::unresolved(ExprType::INT));
    assert_ne!(ExprValue::unresolved(ExprType::INT), ExprValue::unresolved(ExprType::FLOAT));
    assert_ne!(ExprValue::unresolved(ExprType::INT), ExprValue::Null);
}

#[test] fn value_unresolved_is_unresolved() {
