...
pub fn validate_structure(jt: &JobTemplate, limits: &super::EffectiveLimits, rules: &EffectiveRules, errors: &mut ValidationErrors) {
    ...
    if let Some(exts) = &jt.extensions {
        if exts.is_empty() {
            errors.add(&path_field(&root, "extensions"), "if provided, must contain at least one element.");
        }
    }
    ...
    if let Some(desc) = &jt.description {
        ...
        if desc.0.chars().count() > limits.max_description_len {
            errors.add(&dp, format!("exceeds {} characters.", limits.max_description_len));
        }
    ...
    if let Some(params) = &jt.parameter_definitions {
        ...
        for (i, p) in params.iter().enumerate() {
            ...
            if !rules.allowed_job_param_types.contains(&p.job_param_type()) {
                errors.add(&p_path, format!("parameter type '{}' is not allowed.", p.type_name()));
            }
            if let Err(param_errors) = p.validate_definition(limits) {
                for e in param_errors { errors.add(&p_path, e); }
            }
        }
    }
    ...
    if let Some(envs) = &jt.job_environments {
        ...
        for (i, env) in envs.iter().enumerate() {
            if !env_names.insert(env.name.clone()) {
                errors.add(&path_index(&envs_path, i), format!("duplicate environment name: '{}'", env.name));
            }
        }
    }
    for (i, step) in jt.steps.iter().enumerate() {
        if let Some(envs) = &step.step_environments {
            let envs_path = path_field(&vec![PathElement::Field("steps".into()), PathElement::Index(i)], "stepEnvironments");
            if envs.is_empty() {
                errors.add(&envs_path, "must not be empty.");
            }
            for (j, env) in envs.iter().enumerate() {
                if !env_names.insert(env.name.clone()) {
                    errors.add(&path_index(&envs_path, j), format!("duplicate environment name: '{}'", env.name));
                }
            }
        }
    }
    ...
    for (i, step) in jt.steps.iter().enumerate() {
        ...
        if !step_names.insert(name.clone()) {
            errors.add(&path_field(&step_path, "name"), format!("duplicate step name: '{name}'"));
        }
        if name.is_empty() {
            errors.add(&path_field(&step_path, "name"), "must not be empty.");
        }
        if name.chars().any(|c| c.is_control()) {
            errors.add(&path_field(&step_path, "name"), "contains control characters.");
        }
        if name.contains("{{") {
            errors.add(&path_field(&step_path, "name"), "must not contain format string expressions.");
        }
        if let Some(desc) = &step.description {
            let dp = path_field(&step_path, "description");
            if desc.0.chars().count() > limits.max_description_len { errors.add(&dp, format!("exceeds {} characters.", limits.max_description_len)); }
            if has_control_chars(&desc.0) { errors.add(&dp, "contains control characters."); }
        }
        // Must have script or SimpleAction
        if step.script.is_none() && step.bash.is_none() && step.python.is_none()
            && step.cmd.is_none() && step.powershell.is_none() && step.node.is_none() {
        ...
        if let Some(deps) = &step.dependencies {
            ...
            for (j, dep) in deps.iter().enumerate() {
                ...
                if !step_names.contains(&dep.depends_on) && !all_step_names.contains(&dep.depends_on) {
                    errors.add(&dep_path, format!("dependency '{}' not found.", dep.depends_on));
                }
                if !dep_names.insert(&dep.depends_on) {
                    errors.add(&dep_path, format!("duplicate dependency '{}'.", dep.depends_on));
                }
            }
        }
        ...
        if let Some(hr) = &step.host_requirements {
            validate_host_requirements(hr, &path_field(&step_path, "hostRequirements"), rules, errors);
        }

        // Parameter space
        if let Some(ps) = &step.parameter_space {
            validate_step_param_space(ps, &path_field(&step_path, "parameterSpace"), limits, rules, errors);
        }
        ...
        if let Some(script) = &step.script {
            ...
            let file_names: HashSet<String> = script.embedded_files.as_ref()
            ...
            let all_fs: Vec<&openjd_expr::FormatString> = {
                ...
                if let Some(args) = &script.actions.on_run.args { v.extend(args.iter()); }
                v
            };
            for fs in &all_fs {
                for name in fs.expression_names() {
                    if let Some(file_name) = name.strip_prefix("Task.File.") {
                        if !file_names.contains(file_name) {
                            errors.add(&script_path, format!("references undefined embedded file '{file_name}'."));
                        }
                    }
                }
            }
...
pub fn validate_single_environment(env: &Environment, limits: &super::EffectiveLimits, rules: &EffectiveRules, path: &[PathElement], errors: &mut ValidationErrors) {
    ...
    if env.name.len() > limits.max_env_name_len {
        errors.add(&name_path, format!("exceeds {} characters.", limits.max_env_name_len));
    }
    ...
    if let Some(vars) = &env.variables {
        ...
        for (name, value) in vars {
            ...
            if value.raw().chars().count() > limits.max_description_len {
                errors.add(&var_path, format!("value exceeds {} characters.", limits.max_description_len));
            }
        }
    }
    ...
    if let Some(script) = &env.script {
        ...
        if let Some(action) = &script.actions.on_enter {
            validate_action(action, &path_field(&actions_path, "onEnter"), limits, rules, errors);
        }
        if let Some(action) = &script.actions.on_exit {
            validate_action(action, &path_field(&actions_path, "onExit"), limits, rules, errors);
        }
        if let Some(files) = &script.embedded_files {
            let files_path = path_field(&script_path, "embeddedFiles");
            if files.is_empty() {
...
fn validate_action(action: &Action, path: &[PathElement], limits: &super::EffectiveLimits, rules: &EffectiveRules, errors: &mut ValidationErrors) {
    let cmd = action.command.raw();
    if cmd.is_empty() {
        errors.add(&path_field(path, "command"), "must not be empty.");
    }
    if cmd.len() > limits.max_command_len {
        errors.add(&path_field(path, "command"), format!("exceeds {} characters.", limits.max_command_len));
    }
    ...
    if let Some(args) = &action.args {
        ...
        for (i, arg) in args.iter().enumerate() {
            if arg.raw().chars().any(|c| c.is_control() && c != '\n' && c != '\r') {
                errors.add(&path_index(&args_path, i), "contains control characters.");
            }
        }
    }
    if let Some(cancel) = &action.cancelation {
        if let CancelationMode::NotifyThenTerminate { notify_period_in_seconds } = cancel {
            if let Some(period) = notify_period_in_seconds {
                ...
                if !period.has_complex_expressions() && !raw.contains("{{") {
                    ...
                    errors.add(path, "format strings in notifyPeriodInSeconds are not allowed.");
                }
            }
        }
    }
...
fn validate_host_requirements(hr: &HostRequirements, path: &[PathElement], rules: &EffectiveRules, errors: &mut ValidationErrors) {
    let has_amounts = hr.amounts.as_ref().map_or(false, |a| !a.is_empty());
    let has_attrs = hr.attributes.as_ref().map_or(false, |a| !a.is_empty());
    if !has_amounts && !has_attrs {
        errors.add(path, "must have at least one of amounts or attributes.");
    }
    let total = hr.amounts.as_ref().map_or(0, |a| a.len()) + hr.attributes.as_ref().map_or(0, |a| a.len());
    if total > 50 {
    ...
    if let Some(amounts) = &hr.amounts {
        ...
        for (i, amt) in amounts.iter().enumerate() {
            ...
            if amt.name.len() > 100 {
                errors.add(&amt_path, format!("name '{}' exceeds 100 characters.", amt.name));
            }
            if !AMOUNT_CAP_RE.is_match(&amt.name) {
                errors.add(&amt_path, format!("name '{}' does not match capability name pattern.", amt.name));
            }
            check_capability_reserved_scope(&amt.name, STANDARD_AMOUNT_CAPABILITIES, &amt_path, errors);
            ...
            for (field, fs) in [("min", &amt.min), ("max", &amt.max)] {
                if let Some(fs) = fs {
                    if (fs.has_complex_expressions() || fs.raw().contains("{{")) && !rules.allow_fmtstring_in_numeric_fields {
                        errors.add(&path_field(&amt_path, field), "format strings are not allowed.");
                    }
                }
            }
            let min_val = amt.min.as_ref().and_then(|fs| fs.raw().trim().parse::<f64>().ok());
            let max_val = amt.max.as_ref().and_then(|fs| fs.raw().trim().parse::<f64>().ok());
            if let Some(min) = min_val { if min < 0.0 { errors.add(&path_field(&amt_path, "min"), "must be non-negative."); } }
            if let Some(max) = max_val { if max <= 0.0 { errors.add(&path_field(&amt_path, "max"), "must be positive."); } }
            if let (Some(min), Some(max)) = (min_val, max_val) {
                if min > max { errors.add(&amt_path, format!("min ({min}) > max ({max}).")); }
            }
        }
    }
    ...
    if let Some(attrs) = &hr.attributes {
        ...
        for (i, attr) in attrs.iter().enumerate() {
            ...
            if !names.insert(attr.name.to_lowercase()) {
                errors.add(&attr_path, format!("duplicate attribute name '{}'.", attr.name));
            }
            if attr.name.len() > 100 {
                errors.add(&attr_path, format!("name '{}' exceeds 100 characters.", attr.name));
            }
            if !ATTR_CAP_RE.is_match(&attr.name) {
                errors.add(&attr_path, format!("name '{}' does not match capability name pattern.", attr.name));
            }
            check_capability_reserved_scope(&attr.name, STANDARD_ATTRIBUTE_CAPABILITIES, &attr_path, errors);
            ...
            for (field, vals) in [("anyOf", &attr.any_of), ("allOf", &attr.all_of)] {
                if let Some(vals) = vals {
                    ...
                    if vals.is_empty() { errors.add(&field_path, "must not be empty."); }
                    if vals.len() > 50 { errors.add(&field_path, "exceeds 50 elements."); }
                    for (j, v) in vals.iter().enumerate() {
                        let v_path = path_index(&field_path, j);
                        let s = v.raw();
                        if s.is_empty() { errors.add(&v_path, "must not be empty."); }
                        if s.len() > 100 { errors.add(&v_path, "exceeds 100 characters."); }
                        if !s.is_empty() && !ATTR_VALUE_RE.is_match(s) && v.is_literal() {
                            errors.add(&v_path, format!("value '{}' contains invalid characters.", s));
                        }
                    }
                }
            }
            ...
            let is_single_valued = attr_lower == "attr.worker.os.family" || attr_lower == "attr.worker.cpu.arch";
            if is_single_valued {
                if let Some(vals) = &attr.all_of {
                    if vals.len() > 1 { errors.add(&path_field(&attr_path, "allOf"), "single-valued attribute cannot have more than 1 element."); }
                }
            }
                    ...
                    if let Some(vals) = vals {
                        for v in vals {
                            if v.is_literal() && !valid.iter().any(|vv| vv.eq_ignore_ascii_case(v.raw())) {
                                errors.add(&path_field(&attr_path, field), format!("value '{}' is not valid for attr.worker.os.family.", v.raw()));
                            }
                        }
                    }
                }
            ...
            if attr_lower == "attr.worker.cpu.arch" {
                ...
                for (field, vals) in [("anyOf", &attr.any_of), ("allOf", &attr.all_of)] {
                    if let Some(vals) = vals {
                        for v in vals {
                            if v.is_literal() && !valid.iter().any(|vv| vv.eq_ignore_ascii_case(v.raw())) {
                                errors.add(&path_field(&attr_path, field), format!("value '{}' is not valid for attr.worker.cpu.arch.", v.raw()));
                            }
                        }
                    }
                }
...
fn validate_step_param_space(space: &StepParameterSpaceDefinition, path: &[PathElement], limits: &super::EffectiveLimits, rules: &EffectiveRules, errors: &mut ValidationErrors) {
    ...
    for (i, p) in space.task_parameter_definitions.iter().enumerate() {
        ...
        if !names.insert(p.name().to_string()) {
            errors.add(&p_path, format!("duplicate task parameter name '{}'.", p.name()));
        }
        // Check type is allowed
        if !rules.allowed_task_param_types.contains(&p.task_param_type()) {
            errors.add(&p_path, format!("task parameter type '{}' is not allowed.", p.task_param_type()));
        }
...
fn validate_combination_expr(expr: &str, param_names: &HashSet<String>, path: &[PathElement], errors: &mut ValidationErrors) {
    ...
    for ch in expr.chars() {
        match ch {
            '(' => depth += 1,
            ')' => { depth -= 1; if depth < 0 { errors.add(path, "unmatched ')'."); return; } }
            _ => {}
        }
    }
    if depth != 0 { errors.add(path, "unmatched '('."); return; }

    // Tokenize into names and operators
    #[derive(Debug, PartialEq)]
    enum Token { Name(String), Star, LParen, RParen, Comma }
    ...
    if !current.is_empty() { tokens.push(Token::Name(current)); }
    ...
    for name in &all_names {
        if !seen.insert(name.clone()) {
            errors.add(path, format!("parameter '{name}' appears more than once in combination."));
        }
    }
    ...
    for name in param_names {
        if !found_set.contains(name) {
            errors.add(path, format!("parameter '{name}' missing from combination expression."));
        }
    }
}

fn validate_embedded_files(files: &[EmbeddedFile], path: &[PathElement], errors: &mut ValidationErrors) {
    let mut names = HashSet::new();
    for (i, f) in files.iter().enumerate() {
        let f_path = path_index(path, i);
        if !names.insert(&f.name) {
            errors.add(&f_path, format!("duplicate embedded file name '{}'.", f.name));
        }
        if f.name.is_empty() || !f.name.chars().next().map_or(false, |c| c.is_alphabetic() || c == '_')
            || !f.name.chars().all(|c| c.is_alphanumeric() || c == '_') {
            errors.add(&path_field(&f_path, "name"), format!("'{}' is not a valid identifier.", f.name));
        }
        if f.data.is_none() {
            errors.add(&f_path, format!("embedded file '{}' is missing 'data' field.", f.name));
        }
        ...
        if let Some(filename) = &f.filename {
            ...
            if fname.contains('/') || fname.contains('\\') {
                errors.add(&path_field(&f_path, "filename"), "must not contain path separators.");
            }
        }
    }
}
...
fn detect_dependency_cycles(steps: &[StepTemplate], errors: &mut ValidationErrors) {
    ...
    for step in steps {
        let name = step.name.clone();
        let deps: Vec<String> = step.dependencies.as_ref()
    ...
    for name in &step_names { in_degree.insert(name.clone(), 0); }
    for (_, deps) in &adj {
        for dep in deps {
            if let Some(d) = in_degree.get_mut(dep) { *d += 1; }
        }
    }
...
fn validate_task_param_range(p: &TaskParameterDefinition, path: &[PathElement], limits: &super::EffectiveLimits, errors: &mut ValidationErrors) {
    match p {
        TaskParameterDefinition::INT(tp) => {
            match &tp.range {
                IntRange::List(items) => {
                    if items.is_empty() { errors.add(path, format!("INT parameter '{}' range must not be empty.", tp.name)); }
                    if items.len() > limits.max_task_param_range_len { errors.add(path, format!("INT parameter '{}' range exceeds {} elements.", tp.name, limits.max_task_param_range_len)); }
                }
                IntRange::Expression(expr) => {
                    ...
                    if !raw.contains("{{") {
                        match raw.parse::<openjd_expr::RangeExpr>() {
                            ...
                            Err(e) => errors.add(path, format!("INT parameter '{}' range expression error: {e}", tp.name)),
                        }
                    }
                }
            }
        }
        TaskParameterDefinition::FLOAT(tp) => {
            match &tp.range {
                FloatRange::List(items) => {
                    if items.is_empty() { errors.add(path, format!("FLOAT parameter '{}' range must not be empty.", tp.name)); }
                    if items.len() > limits.max_task_param_range_len { errors.add(path, format!("FLOAT parameter '{}' range exceeds {} elements.", tp.name, limits.max_task_param_range_len)); }
                }
        ...
        TaskParameterDefinition::STRING(tp) => {
            if let StringRange::List(items) = &tp.range {
                if items.is_empty() { errors.add(path, format!("STRING parameter '{}' range must not be empty.", tp.name)); }
                if items.len() > limits.max_task_param_range_len { errors.add(path, format!("STRING parameter '{}' range exceeds {} elements.", tp.name, limits.max_task_param_range_len)); }
            }
        }
        TaskParameterDefinition::PATH(tp) => {
            if let StringRange::List(items) = &tp.range {
                if items.is_empty() { errors.add(path, format!("PATH parameter '{}' range must not be empty.", tp.name)); }
                if items.len() > limits.max_task_param_range_len { errors.add(path, format!("PATH parameter '{}' range exceeds {} elements.", tp.name, limits.max_task_param_range_len)); }
                for (i, item) in items.iter().enumerate() {
                    if item.raw().is_empty() {
                        errors.add(path, format!("PATH parameter '{}' range[{i}] must not be empty.", tp.name));
                    }
                }
            }
        }
        TaskParameterDefinition::ChunkInt(tp) => {
            match &tp.range {
                IntRange::List(items) => {
                    if items.is_empty() { errors.add(path, format!("CHUNK[INT] parameter '{}' range must not be empty.", tp.name)); }
                    if items.len() > limits.max_task_param_range_len { errors.add(path, format!("CHUNK[INT] parameter '{}' range exceeds {} elements.", tp.name, limits.max_task_param_range_len)); }
                }
                IntRange::Expression(expr) => {
                    if !expr.raw().contains("{{") {
                        if let Err(e) = expr.raw().parse::<openjd_expr::RangeExpr>() {
                            errors.add(path, format!("CHUNK[INT] parameter '{}' range expression error: {e}", tp.name));
                        }
                    }
                }
            }
